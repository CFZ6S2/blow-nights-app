  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 pb-32">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-[20%] -right-[10%] w-[40%] h-[40%] bg-amber-600/10 blur-[120px] rounded-full" />
      </div>
          ...
          <div className="text-center py-16 space-y-4">
            ...
            {tab === 'valid' && (
              <button
                ...
                className="px-6 py-3 rounded-2xl bg-amber-600 text-white text-xs font-black uppercase tracking-widest"
              >
          ...
          <div className="space-y-3">
            {displayTickets.map((ticket: any) => (
              <motion.button
                ...
                className={`w-full bg-slate-50 backdrop-blur-xl border rounded-3xl p-5 text-left transition-all ${
                  ticket.status === 'valid'
        ...
        {selectedTicket && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-sm bg-slate-900 border border-slate-200 rounded-[3rem] p-8 space-y-6 shadow-[0_40px_100px_rgba(0,0,0,0.8)]"
            >
              ...
              <div id="ticket-content" className="print:text-black">
                ...
                <div className="space-y-2 print:hidden">
                  <button onClick={() => window.print()} className="w-full py-3 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-colors">
                    <span className="material-icons text-sm">picture_as_pdf</span>
                    {t('wallet.download_pdf')}
                  </button>
                  <button disabled className="w-full py-3 bg-black border border-slate-200 text-slate-500 font-bold rounded-xl text-xs flex items-center justify-center gap-2 opacity-50 cursor-not-allowed">
                    <span className="material-icons text-sm">account_balance_wallet</span>
                    {t('wallet.add_to_apple_wallet')}
                  </button>
                </div>

                <div className="mt-4 p-4 bg-white print:bg-white print:border print:border-slate-300 rounded-xl text-[8px] text-slate-500 print:text-slate-800 leading-tight space-y-2 max-h-32 overflow-y-auto print:max-h-none text-left">
              ...
              <button
                onClick={() => setSelectedTicket(null)}
                className="w-full mt-6 py-4 rounded-2xl bg-slate-50 text-slate-400 font-black uppercase tracking-[0.2em] text-xs hover:bg-slate-100 transition-all print:hidden"
              >
                {t('wallet.close')}
              </button>
            </motion.div>
