...
function ChatDetailContent() {
  ...
  if (loading || !otherUser) return (
    <div className="flex items-center justify-center min-h-screen bg-white">
      <motion.div 
        ...
        className="h-10 w-10 border-t-2 border-amber-500 rounded-full"
      />
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-screen bg-white text-slate-900 overflow-hidden">
      {/* Header Premium */}
      <header className="px-6 py-6 bg-slate-900/40 backdrop-blur-3xl border-b border-slate-200 flex items-center gap-4 z-40">
        <motion.button 
          whileTap={{ scale: 0.9 }}
          onClick={() => { triggerHaptic(5); router.back(); }} 
          className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-200"
        >
        ...
        <div className="relative">
          <motion.div 
            ...
            className="w-12 h-12 rounded-2xl overflow-hidden border border-slate-200 shadow-2xl transition-all duration-500"
          >
            ...
            <AnimatePresence mode="wait">
              {isOtherTyping ? (
                <motion.p 
                  ...
                  exit={{ opacity: 0, x: 5 }}
                  className="text-[9px] font-black uppercase tracking-widest text-amber-400 animate-pulse"
                >
                  {t('chat.typing')}
                </motion.p>
          ...
          {showOptions && (
            <>
              <motion.div 
                ...
                className="fixed inset-0 bg-slate-200 z-30"
              />
              <motion.div 
                initial={{ opacity: 0, scale: 0.9, y: -20 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.9, y: -20 }}
                className="absolute top-24 right-6 w-56 bg-slate-900/90 backdrop-blur-2xl border border-slate-200 rounded-3xl p-2 z-40 shadow-2xl"
              >
                <button onClick={() => { triggerHaptic(10); router.push(cityPath(`/profile/view?id=${otherUser.id || searchParams.get('otherId')}`)); }} className="w-full p-4 flex items-center gap-3 text-sm font-bold text-slate-200 hover:bg-slate-50 rounded-2xl transition-all">
            ...
            return (
              ...
              >
                <div className={`max-w-[80%] p-4 rounded-3xl relative group transition-all duration-300 ${
                  isMe 
                  ? 'bg-gradient-to-br from-amber-600 to-blue-600 text-white rounded-tr-none shadow-[0_10px_20px_rgba(217,70,239,0.15)]' 
                  : 'bg-slate-800/80 backdrop-blur-xl border border-slate-200 rounded-tl-none text-slate-100 shadow-xl'
                }`}>
                  {/* Bubble Tail */}
                  <div className={`absolute top-0 w-4 h-4 ${
                    isMe 
                    ? 'right-[-8px] bg-blue-600 [clip-path:polygon(0_0,0_100%,100%_0)]' 
                  ...
                  {msg.type === 'image' ? (
                    <div className="space-y-2">
                      <motion.img 
                        ...
                        src={msg.url} 
                        alt="Media" 
                        className="w-72 max-w-full rounded-2xl cursor-pointer border border-slate-200 shadow-2xl"
                      />
        ...
        {isOtherTyping && (
          <div className="flex justify-start">
            <div className="bg-slate-800/40 px-6 py-4 rounded-3xl rounded-tl-none flex gap-1.5">
              {[0, 1, 2].map(i => (
                <motion.div key={i} animate={{ y: [0, -5, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: i * 0.1 }} className="w-1.5 h-1.5 bg-amber-500 rounded-full" />
              ))}
            </div>
          </div>
        )}
      ...
      {/* Input v33 MediaFlow */}
      <footer className="p-6 pb-12 bg-slate-900/80 backdrop-blur-3xl border-t border-slate-200 relative">
        {uploading && (
          <motion.div 
            initial={{ width: 0 }}
            animate={{ width: `${progress}%` }}
            className="absolute top-0 left-0 h-1 bg-amber-500 shadow-[0_0_15px_rgba(217,70,239,0.8)] z-50"
          />
        )}
        ...
        <form onSubmit={handleSend} className="max-w-4xl mx-auto flex items-end gap-3">
          ...
          <motion.button
            ...
            className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-200 hover:bg-slate-100 transition-all text-slate-400"
          >
            <span className="material-icons">add_a_photo</span>
          </motion.button>

          <div className="flex-1 bg-slate-50 rounded-[2rem] border border-slate-200 p-2 flex items-end gap-2 focus-within:border-amber-500/30 transition-all relative overflow-hidden">
            {isRecording ? (
          ...
          <AnimatePresence mode="wait">
            <motion.button 
              ...
              className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                ...
                : (newMessage.trim() ? 'bg-amber-600 text-white shadow-lg shadow-amber-500/20' : 'bg-slate-50 text-slate-400')
              }`}
            >
