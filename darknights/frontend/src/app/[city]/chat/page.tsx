...
export default function ChatListPage() {
  ...
  if (authLoading || loading) return (
    <div className="flex flex-col min-h-screen bg-white text-slate-900 p-4">
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-slate-900 p-4">
      <header className="py-6 px-4 flex justify-between items-center">
        <button onClick={() => router.push(cityPath())} className="p-2 hover:bg-slate-100 rounded-full transition-all">
        ...
        <h1 className="text-2xl font-bold">{t('chat.title')}</h1>
        {isSuperAdmin ? (
          <Link href={cityPath('/profile')} className="p-2 hover:bg-slate-100 rounded-full transition-all">
        ...
        {chats.length === 0 ? (
          ...
          chats.map((chat) => (
            <Link 
              ...
              className="flex items-center gap-4 p-4 bg-slate-50 border border-slate-200 rounded-3xl hover:bg-slate-100 transition-all group"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline">
                  <h3 className="font-bold text-white group-hover:text-blue-400 transition-colors">{chat.otherUser?.nick}</h3>
