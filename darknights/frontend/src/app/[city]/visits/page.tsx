...
export default function VisitsPage() {
  ...
  if (authLoading || loading) {
    return (
      <div className="flex flex-col min-h-screen bg-white text-slate-900 p-8">
        <header className="py-8 flex items-center gap-6">
          <div className="w-12 h-12 rounded-2xl bg-slate-50 animate-pulse" />
          <div className="h-8 w-40 bg-slate-50 rounded-xl animate-pulse" />
        </header>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-slate-900 pb-32 overflow-x-hidden">
      <AnimatePresence>
        {selectedProfileId && (
      ...
      <header className="sticky top-0 z-40 bg-white/60 backdrop-blur-3xl border-b border-slate-200 px-8 py-8">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          <div className="flex items-center gap-6">
            <motion.button 
              whileTap={{ scale: 0.9 }}
              onClick={() => { triggerHaptic(5); router.back(); }} 
              className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center hover:bg-slate-100 transition-all"
            >
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            <span className="text-amber-400 font-black text-xs">{visits.length}</span>
            <span className="material-icons text-amber-500 text-sm">visibility</span>
          </div>
        </div>
      </header>
          ...
          <div className="grid gap-3">
            {visits.map((visit, idx) => (
              <motion.div
                ...
                className={`relative flex items-center gap-5 p-5 bg-slate-50 border border-slate-200 rounded-[2.5rem] transition-all overflow-hidden ${isPremium ? 'hover:bg-slate-100 active:scale-[0.98]' : 'cursor-pointer'}`}
              >
                {!isPremium && idx > 2 && (
                ...
                <div className={`relative w-20 h-20 rounded-[1.8rem] overflow-hidden border-2 transition-all ${isPremium ? 'border-amber-500/30' : 'border-slate-800'}`}>
                  <img 
