      ...
      <main className="relative z-10 p-6 md:p-12 max-w-2xl mx-auto space-y-8">
        ...
        <header className="flex flex-col items-center text-center space-y-4 pt-8">
          <div className="relative group">
            <motion.div 
              ...
              className="w-32 h-32 rounded-[2.5rem] overflow-hidden border-2 border-amber-500/30 shadow-[0_20px_50px_rgba(217,70,239,0.2)] p-1 bg-gradient-to-br from-amber-500 to-blue-600"
            >
              <img 
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs font-black uppercase tracking-[0.2em] mt-1">
              {profile.role === 'rrpp' ? (
                <span className="text-amber-400">{t('profile.role_promoter')}</span>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: t('profile.likes'), value: stats.likes, icon: 'favorite', color: 'text-amber-400' },
            { label: t('profile.matches'), value: stats.matches, icon: 'bolt', color: 'text-yellow-400' }
          ].map((stat, i) => (
            <motion.div 
              key={i}
              whileHover={{ y: -5 }}
              className="bg-slate-50 backdrop-blur-xl border border-slate-200 p-4 rounded-3xl flex flex-col items-center space-y-1"
            >
        ...
        {/* City Selector */}
        <section className="bg-slate-50 backdrop-blur-xl border border-slate-200 p-6 rounded-[2.5rem] space-y-4">
          <div>
          ...
          <div className="flex items-center justify-between">
            ...
            <motion.button
              ...
              className={`px-6 py-3 rounded-2xl font-black text-[10px] tracking-widest transition-all duration-500 flex items-center gap-3 border ${
                ...
                : 'bg-slate-800 border-slate-200 text-slate-400'
              }`}
            >
        ...
        {/* Ghost Mode Toggle */}
        <section className="bg-slate-50 backdrop-blur-xl border border-slate-200 p-6 rounded-[2.5rem] relative overflow-hidden group">
          <div className="flex items-center justify-between relative z-10">
            ...
            <button
              ...
              className={`w-14 h-8 rounded-full transition-all relative ${
                profile?.ghostMode ? 'bg-blue-600' : 'bg-slate-800'
              }`}
            >
              <motion.div 
        ...
        <section className="bg-slate-50 backdrop-blur-xl border border-slate-200 p-6 rounded-[2.5rem] relative overflow-hidden group">
          <div className="flex items-center justify-between relative z-10">
            ...
            <button
              ...
              className={`w-14 h-8 rounded-full transition-all relative ${
                profile?.nsfwBlur !== false ? 'bg-amber-600' : 'bg-slate-800'
              }`}
            >
              <motion.div
        ...
        <section className="bg-slate-50 backdrop-blur-xl border border-slate-200 p-6 rounded-[2.5rem] relative overflow-hidden group">
          <div className="flex items-center justify-between relative z-10">
            ...
            <button
              ...
              className={`w-14 h-8 rounded-full transition-all relative ${
                profile?.notificationsEnabled ? 'bg-amber-600' : 'bg-slate-800'
              }`}
            >
              <motion.div 
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-amber-600 to-blue-600 text-white font-[1000] py-6 rounded-[2.5rem] shadow-2xl flex items-center justify-center gap-4 active:scale-95 transition-all uppercase tracking-[0.2em] text-[10px]"
        >
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-end">
            ...
            <Link href="/setup-profile" prefetch={false} className="text-[10px] font-black text-amber-500 uppercase tracking-widest">{t('profile.manage')}</Link>
          </div>
          <div className="grid grid-cols-3 gap-2">
            ...
            {allPhotos.length < 6 && (
              <Link 
                href="/setup-profile"
                className="aspect-square rounded-2xl bg-slate-50 border border-dashed border-slate-200 flex flex-col items-center justify-center gap-1 text-slate-500 hover:bg-slate-100 transition-all"
              >
        ...
        <section className="space-y-3">
          {[
            ...
            { 
              ...
              href: '#', 
              color: profile.verified ? 'text-blue-500' : profile.verificationStatus === 'pending' ? 'text-amber-500' : 'text-amber-500',
              onClick: async () => {
            ...
            { label: t('profile.venue_panel'), icon: 'storefront', href: '/venue-admin', color: 'text-amber-400' },
            { label: t('profile.scan_tickets'), icon: 'qr_code_scanner', href: '/scanner', color: 'text-amber-400' },
            ...
            if (item.onClick) {
              return (
                <button 
                  ...
                  className="w-full p-5 bg-slate-50 backdrop-blur-xl border border-slate-200 rounded-3xl flex items-center justify-between group hover:bg-slate-100 transition-all text-left"
                >
                  {content}
                </button>
              );
            ...
            return (
              <Link 
                key={i}
                href={item.href}
                className="w-full p-5 bg-slate-50 backdrop-blur-xl border border-slate-200 rounded-3xl flex items-center justify-between group hover:bg-slate-100 transition-all"
              >
                {content}
              </Link>
            );
          ...
          <button 
            ...
            disabled={isDeleting}
            className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center gap-3 text-slate-600 text-[10px] font-black uppercase tracking-widest hover:text-red-500 hover:bg-red-500/5 transition-all mt-8"
          >
        ...
        {showDeleteModal && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="w-full max-w-sm bg-slate-900 border border-slate-200 rounded-[3rem] p-10 space-y-8 shadow-[0_40px_100px_rgba(0,0,0,0.8)] text-center"
            >
              ...
              <div className="space-y-3">
                ...
                <button 
                  ...
                  className="w-full py-5 rounded-2xl bg-slate-50 text-slate-400 font-black uppercase tracking-[0.2em] text-xs hover:bg-slate-100 transition-all"
                >
                  {t('profile.no_stay')}
                </button>
              </div>
