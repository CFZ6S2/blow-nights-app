            ...
            <div className="flex items-center gap-2">
              {selectedVenue?.subscriptionTier && !isAdmin && (
                <button
                  ...
                  className="px-4 py-3 rounded-2xl border border-slate-200 hover:bg-slate-50 text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-colors"
                >
              ...
              {(selectedVenue?.subscriptionTier === 'ticketing' || isAdmin) && (
                <Link
                  href="/scanner"
                  className="px-4 py-3 rounded-2xl bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest flex items-center gap-2"
                >
        ...
        {selectedVenue && !selectedVenue.subscriptionTier && selectedVenue.ownerId !== 'community' && !isAdmin && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-slate-900 border border-slate-700 p-4 rounded-xl flex flex-col items-center text-center">
              ...
              <p className="text-xl font-black text-amber-400">30€<span className="text-sm text-slate-500">/mes</span></p>
              <p className="text-xs text-slate-400 mt-2 mb-4">Presencia en el mapa y configuración básica.</p>
              <button onClick={() => handleSubscription('basico')} disabled={checkingOut} className="w-full mt-auto py-2 bg-slate-100 hover:bg-slate-100 rounded-lg text-sm font-bold">Activar</button>
            </div>
            <div className="bg-gradient-to-br from-slate-800/40 to-blue-900/40 border border-amber-500/50 p-4 rounded-xl flex flex-col items-center text-center">
              <h3 className="font-black text-lg">Promo</h3>
              <p className="text-xl font-black text-amber-400">60€<span className="text-sm text-slate-500">/mes</span></p>
              <p className="text-xs text-slate-400 mt-2 mb-4">Envía notificaciones push promocionales a usuarios cercanos.</p>
              <button onClick={() => handleSubscription('promo')} disabled={checkingOut} className="w-full mt-auto py-2 bg-amber-600 hover:bg-amber-500 rounded-lg text-sm font-bold shadow-[0_0_15px_rgba(217,70,239,0.5)]">Activar</button>
            </div>
            <div className="bg-slate-900 border border-slate-700 p-4 rounded-xl flex flex-col items-center text-center">
              <h3 className="font-black text-lg">Ticketing</h3>
              <p className="text-xl font-black text-amber-400">100€<span className="text-sm text-slate-500">/mes</span></p>
              <p className="text-xs text-slate-400 mt-2 mb-4">Gestión de eventos, RRPP y escáner de entradas.</p>
              <button onClick={() => handleSubscription('ticketing')} disabled={checkingOut} className="w-full mt-auto py-2 bg-slate-100 hover:bg-slate-100 rounded-lg text-sm font-bold">Activar</button>
            </div>
          </div>
        )}

        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            <button onClick={() => setTab('overview')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'overview' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>{t('tab_overview')}</button>
            <button onClick={() => setTab('config')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'config' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>{t('tab_config')}</button>
            <button onClick={() => setTab('catalog')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'catalog' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>{t('tab_catalog')}</button>
            <button onClick={() => setTab('events')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'events' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>{t('tab_events')}</button>
            <button onClick={() => setTab('tickets')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'tickets' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>{t('tab_ticketing')}</button>
            <button onClick={() => setTab('promos')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'promos' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>{t('tab_promos')}</button>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            {selectedVenue.subscriptionTier === 'ticketing' || isAdmin ? (
              ...
              <div className="text-center p-8 bg-slate-900 border border-slate-800 rounded-2xl">
                ...
                <button onClick={() => handleSubscription('ticketing')} disabled={checkingOut} className="px-6 py-2 bg-amber-600 rounded-xl font-bold">Actualizar Plan</button>
              </div>
            )}
          </motion.div>
        )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'Entradas activas', value: validTickets, icon: 'local_activity', color: 'text-amber-400' },
                { label: 'Hora punta', value: peakHour ? `${peakHour[0]}:00` : '-', icon: 'schedule', color: 'text-blue-400' },
              ].map((stat, i) => (
                <motion.div
                  key={i}
                  whileHover={{ y: -3 }}
                  className="bg-slate-50 backdrop-blur-xl border border-slate-200 p-5 rounded-3xl space-y-2"
                >
                  ...
                  {checkins.map((c) => (
                    <div key={c.id} className="flex items-center justify-between py-2 border-b border-slate-200 last:border-0">
                      ...
                      <span className={`text-[9px] font-black uppercase tracking-widest ${
                        c.visibility === 'ghost' ? 'text-blue-400' : 'text-slate-500'
                      }`}>
            ...
            {['promo', 'ticketing'].includes(selectedVenue?.subscriptionTier) || isAdmin ? (
              <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4">
                ...
                <textarea
                  ...
                  rows={3}
                  maxLength={200}
                  className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-4 py-3 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-amber-500/50 resize-none"
                />
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      className="bg-slate-100 border border-slate-200 rounded-xl px-3 py-2 text-xs text-white focus:outline-none"
                    >
              ...
              <div className="text-center p-8 bg-slate-900 border border-slate-800 rounded-2xl">
                ...
                <button onClick={() => handleSubscription('promo')} disabled={checkingOut} className="px-6 py-2 bg-amber-600 rounded-xl font-bold">Actualizar Plan</button>
              </div>
            )}
          </div>
        )}
