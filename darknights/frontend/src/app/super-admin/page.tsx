...
export default function SuperAdminPage() {
  ...
  const ROLE_OPTIONS = [
    ...
    { value: 'admin', label: 'Admin', icon: 'admin_panel_settings', color: 'text-blue-400', redirect: '/admin' },
    { value: 'superadmin', label: 'Super Admin', icon: 'shield', color: 'text-amber-400', redirect: '/super-admin' },
    ...
    { value: 'event_organizer', label: 'Organizador', icon: 'calendar_month', color: 'text-blue-400', redirect: '/organizer' },
  ];
  ...
  if (loading || !isSuperAdmin) return <div className="min-h-screen bg-white" />;

  return (
    <div className="min-h-screen bg-white text-slate-900 p-6 md:p-12 pb-32">
      <header className="mb-8">
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => router.back()} className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center">
            <span className="material-icons">arrow_back</span>
          </button>
          <div>
            <h1 className="text-2xl font-black tracking-tight text-amber-400">{t('super_admin_title')}</h1>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar border-b border-slate-200 pb-2">
          <button onClick={() => setActiveTab('usuarios')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'usuarios' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_users')}</button>
          <button onClick={() => setActiveTab('roles')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'roles' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_roles')}</button>
          <button onClick={() => setActiveTab('ciudades')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'ciudades' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_cities')}</button>
          <button onClick={() => setActiveTab('partners')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'partners' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_partners')}</button>
          <button onClick={() => setActiveTab('facturacion')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'facturacion' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_billing')}</button>
          <button onClick={() => setActiveTab('locales')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'locales' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_venues')}</button>
          <button onClick={() => setActiveTab('eventos')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'eventos' ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_events')}</button>
          <button onClick={() => setActiveTab('rrpps')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'rrpps' ? 'bg-amber-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_rrpps')}</button>
          <button onClick={() => setActiveTab('organizadores')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'organizadores' ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-400'}`}>{t('tab_organizers')}</button>
        </div>
      </header>
      ...
      {activeTab === 'usuarios' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none"
              />
              <button onClick={handleAdminUserSearch} disabled={userSearching} className="bg-amber-600 px-5 py-3 rounded-xl text-sm font-bold hover:bg-amber-500 transition-colors disabled:opacity-50">
            ...
            {userSearchResults.length > 0 ? (
              <div className="space-y-3">
                <p className="text-xs text-amber-400 font-bold uppercase mb-2">Resultados de búsqueda ({userSearchResults.length})</p>
                {userSearchResults.map(u => (
                  ...
                  >
                    ...
                    <span className="text-[10px] uppercase font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded">{u.role || 'user'}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                  ...
                  <select
                    ...
                    className="bg-slate-100 border border-slate-200 rounded-lg px-2 py-1 text-xs outline-none"
                  >
                    ...
                    {allUsers
                      ...
                      >
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <span className="text-[10px] uppercase font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded">{u.role || 'user'}</span>
          ...
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl h-[80vh] flex flex-col">
            ...
            {!selectedUserDetail ? (
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-6">
                <div className="flex items-center gap-4">
                  <img src={selectedUserDetail.fotoUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedUserDetail.nick || 'U')}&background=random`} alt={selectedUserDetail.nick} className="w-24 h-24 rounded-2xl object-cover shadow-lg border border-slate-200" />
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className="px-2 py-1 bg-slate-100 text-white text-[10px] rounded uppercase font-bold">{selectedUserDetail.role || 'user'}</span>
                  ...
                  <div className="bg-slate-100 p-4 rounded-xl border border-slate-200">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <select
                        ...
                        className="bg-slate-200 border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-white outline-none w-full appearance-none"
                      >
                ...
                <div className="space-y-2 mt-8">
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2 border-b border-slate-200 pb-2">Acciones de Moderación</p>
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-slate-800/20 hover:bg-slate-800/40 text-amber-400 border border-amber-500/20 px-4 py-3 rounded-xl text-sm font-bold transition-colors disabled:opacity-50"
                  >
      ...
      {activeTab === 'roles' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none"
              />
              <button onClick={handleRoleSearch} disabled={roleSearching} className="bg-amber-600 px-5 py-3 rounded-xl text-sm font-bold hover:bg-amber-500 transition-colors disabled:opacity-50">
                ...
                {selectedRole === 'cityAdmin' && (
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-green-500 outline-none"
                    >
                ...
                {selectedRole === 'venue' && (
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500 outline-none"
                    >
                ...
                {selectedRole === 'ambassador' && (
                  <div className="space-y-2">
                    ...
                    <div className="max-h-48 overflow-y-auto space-y-1 no-scrollbar">
                      {cities.map(c => (
                        <label key={c.id} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-50 cursor-pointer">
                          <input
                            type="checkbox"
          ...
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4 h-[70vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              ...
              {recentAssignments.map(u => {
                ...
                return (
                  <div key={u.id} className="bg-slate-100 border border-slate-200 p-4 rounded-2xl">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${roleInfo?.color || 'text-slate-400'} bg-slate-50`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {cities.filter(c => c.readyForFranchise).map(c => (
                        <div key={c.id} className="flex justify-between items-center bg-slate-100 rounded-xl px-4 py-2 border border-black/10">
            ...
            {applications.length > 0 && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-slate-900 border-2 border-amber-500/50 rounded-3xl p-6 shadow-2xl">
                <div className="flex items-start gap-4">
                  <span className="text-4xl">💼</span>
                  <div className="w-full">
                    ...
                    <div className="space-y-3">
                      {applications.map(app => (
                        <div key={app.id} className="bg-slate-100 rounded-xl p-4 border border-slate-200">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              <p className="font-bold text-white text-lg">{app.name}</p>
                              <p className="text-xs text-amber-400 font-bold uppercase">{app.cityId === 'other' ? 'Otra Ciudad' : app.cityId}</p>
                            </div>
                          ...
                          <p className="text-xs text-slate-500 italic border-l-2 border-amber-500/30 pl-2 mt-2">{app.experience}</p>
                          ...
                          <div className="mt-4 flex gap-2">
                            <button 
                              ...
                              className="px-4 py-2 bg-amber-600 hover:bg-amber-500 rounded-lg text-xs font-bold uppercase transition-colors"
                            >
                              Marcar Revisada
                            </button>
            ...
            <div className="space-y-3">
              <div className="flex gap-3">
                <input placeholder="City Slug (ej: ibiza)" value={newCitySlug} onChange={e => setNewCitySlug(e.target.value)} className="w-1/2 bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
                <input placeholder="Nombre (ej: Ibiza)" value={newCityName} onChange={e => setNewCityName(e.target.value)} className="w-1/2 bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
              </div>
              
              <div className="flex gap-3">
                <input placeholder="Email del City Manager (Opcional)" type="email" value={newCityManagerEmail} onChange={e => setNewCityManagerEmail(e.target.value)} className="w-1/2 bg-slate-100 border border-amber-500/30 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
                <input placeholder="Email del Ambassador (Opcional)" type="email" value={newCityAmbassadorEmail} onChange={e => setNewCityAmbassadorEmail(e.target.value)} className="w-1/2 bg-slate-100 border border-blue-500/30 rounded-xl px-4 py-3 text-sm focus:border-blue-500 outline-none" />
              </div>

              <input placeholder="Empresa / Nombre Partner (Si no hay manager)" value={newCityPartnerName} onChange={e => setNewCityPartnerName(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
              <input placeholder="Email de Facturación / Partner" type="email" value={newCityPartnerEmail} onChange={e => setNewCityPartnerEmail(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
              <input placeholder="Stripe Connect ID (acct_...)" value={newCityStripeId} onChange={e => setNewCityStripeId(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-blue-500 outline-none" />
              ...
              <button onClick={handleCreateCity} disabled={savingCity} className="w-full bg-amber-600 py-3 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-amber-500 transition-colors mt-2">
          ...
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4 h-[60vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              {cities.map(c => {
                ...
                return (
                  <div key={c.id} className={`bg-slate-100 border ${isMadrid ? 'border-amber-500/50' : 'border-slate-200'} p-4 rounded-2xl`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-lg">{c.name}</h3>
                          {isMadrid && <span className="bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Matriz Directa</span>}
                        ...
                        {!isMadrid && c.license && (
                          <div className="mt-2 bg-slate-50 p-2 rounded-lg text-[10px] text-slate-400">
                      ...
                      {!isMadrid && (
                        <button 
                          ...
                          className="bg-slate-100 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-slate-100"
                        >
                          Asignar ROL
                        </button>
                      )}
                    ...
                    {assigningCityId === c.id && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-4 pt-4 border-t border-slate-200">
                        <div className="flex gap-2">
                          <input 
                            placeholder="Email exacto del usuario..." type="email"
                            value={searchEmail} onChange={e => setSearchEmail(e.target.value)}
                            className="flex-1 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-xs focus:border-amber-500 outline-none"
                          />
      ...
      {activeTab === 'facturacion' && (
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-slate-800/40 to-black border border-amber-500/30 p-6 rounded-3xl">
              <h3 className="text-xs font-bold uppercase tracking-widest text-amber-400 mb-2">Revenue Global Matriz</h3>
              <p className="text-4xl font-black font-mono">14.250 <span className="text-lg text-amber-400">€</span></p>
          ...
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="w-full bg-slate-200 rounded-full h-2 flex">
                  <div className="bg-green-500 h-2 rounded-l-full" style={{ width: '40%' }}></div>
                  <div className="bg-slate-100 h-2 rounded-r-full" style={{ width: '60%' }}></div>
                </div>
              </div>
      ...
      {activeTab === 'locales' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4">
            ...
            <div className="space-y-3">
              <input placeholder="Nombre (ej: Parque del Retiro)" value={newVenueName} onChange={e => setNewVenueName(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
              <select value={newVenueType} onChange={e => setNewVenueType(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none">
              ...
              <div className="flex gap-3">
                <input placeholder="Latitud (ej: 40.4168)" type="number" value={newVenueLat} onChange={e => setNewVenueLat(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
                <input placeholder="Longitud (ej: -3.7038)" type="number" value={newVenueLng} onChange={e => setNewVenueLng(e.target.value)} className="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none" />
              </div>
              <button onClick={handleCreateVenue} disabled={savingVenue} className="w-full bg-amber-600 py-3 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-amber-500 transition-colors">
                Crear Punto
              </button>
            </div>
          </section>
          ...
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4 h-[60vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              {venues.map(v => (
                <div key={v.id} className="bg-slate-100 border border-slate-200 p-4 rounded-2xl">
                  <div className="flex justify-between items-start">
                    <div>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setAssigningVenueId(assigningVenueId === v.id ? null : v.id)} className="bg-slate-100 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-slate-100">
                        Asignar
                      </button>
                  ...
                  {assigningVenueId === v.id && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-4 pt-4 border-t border-slate-200">
                      <div className="flex gap-2">
                        <input placeholder="Buscar por nickname..." value={searchNick} onChange={e => setSearchNick(e.target.value)} className="flex-1 bg-slate-100 border border-slate-200 rounded-lg px-3 py-2 text-xs focus:border-amber-500 outline-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-slate-100 border border-slate-200 rounded-lg px-3 py-3 text-sm focus:border-amber-500 outline-none" 
                />
                <button onClick={handleSearchNewPartner} disabled={searchingPartner} className="bg-amber-600 px-4 py-3 rounded-lg text-sm font-bold hover:bg-amber-500 transition-colors">
                  Buscar
                </button>
              </div>
              ...
              {partnerSearchResults.length > 0 && (
                <div className="mt-4 space-y-2">
                  {partnerSearchResults.map(u => (
                    <div key={u.id} className="flex justify-between items-center bg-slate-100 p-3 rounded-xl">
                      <div>
                      ...
                      {!u.isAssociatePartner ? (
                        ...
                        <span className="text-xs text-amber-400 font-bold">Ya es Partner</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <section className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4 h-[60vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              ...
              {partners.map(p => (
                <div key={p.id} className="bg-slate-100 border border-slate-200 p-4 rounded-2xl flex justify-between items-center">
                  <div>
      ...
      {activeTab === 'rrpps' && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <section className="md:col-span-1 bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4 h-fit">
            ...
            <div className="space-y-3">
              <input 
                ...
                className="w-full bg-slate-100 border border-slate-200 rounded-lg px-3 py-3 text-sm focus:border-amber-500 outline-none" 
              />
              <select
              ...
              <button 
                ...
                className="w-full bg-amber-600 px-4 py-3 rounded-lg text-sm font-bold hover:bg-amber-500 transition-colors disabled:opacity-50"
              >
          ...
          <section className="md:col-span-2 bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4 flex flex-col h-[75vh]">
            <div className="flex justify-between items-center">
              ...
              {promoters.map(p => (
                <div key={p.id} className="bg-slate-100 border border-slate-200 p-4 rounded-2xl flex justify-between items-center">
                  ...
                  <div className="flex flex-col gap-2">
                    <button onClick={() => copyPromoterLink(p.code)} className="bg-slate-100 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-slate-100 transition-colors uppercase">
                      Copiar Enlace
                    </button>
                    <button onClick={() => handleTogglePromoterStatus(p)} className="bg-slate-50 border border-slate-200 text-white px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-slate-100 transition-colors uppercase">
      ...
      {activeTab === 'organizadores' && (
        <div className="space-y-8">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-slate-900 border-2 border-blue-500/50 rounded-3xl p-6 shadow-2xl">
              <div className="flex items-start gap-4">
                <span className="text-4xl">🎪</span>
                <div className="w-full">
