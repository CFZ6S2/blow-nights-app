...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 p-4 pb-24">
      {/* Header */}
      <header className="mb-8">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-black bg-gradient-to-r from-slate-700 to-blue-800 bg-clip-text text-transparent">
              ADMIN DASHBOARD
            </h1>
          ...
          <div className="flex gap-2">
            {isSuperAdmin && (
              <button onClick={() => router.push('/super-admin')} className="bg-amber-600/20 text-amber-400 border border-amber-600/50 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-amber-600/30 transition-all">
                Panel Maestro
              </button>
            )}
            <button onClick={() => router.push('/')} className="bg-slate-50 text-slate-400 border border-slate-200 px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-100 transition-all">
              Volver
            </button>
          </div>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-700/50 rounded-2xl p-4 mb-8 shadow-xl">
        <h2 className="text-lg font-black text-white uppercase tracking-wider mb-4 flex items-center gap-2">
          <span className="material-icons text-amber-500">settings_input_component</span>
          Ecosistema
        </h2>
        
        <div className="flex items-center justify-between p-4 bg-slate-200 rounded-xl border border-slate-200">
          <div>
          ...
          <button
            ...
            className={`w-14 h-8 rounded-full relative transition-colors duration-300 ${platformSettings?.enableDarkNightsBridge ? 'bg-amber-600' : 'bg-slate-700'}`}
          >
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <StatCard label={t('admin.users')} value={stats.totalUsers} icon="people" color="from-amber-500/20 to-amber-500/5" border="border-amber-500/20" />
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'rrpps' && (
          <motion.div key="rrpps" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            ...
            {rrppApps.map(app => (
              <div key={app.id} className="bg-slate-50 backdrop-blur-md border border-amber-500/30 rounded-2xl p-4">
        ...
        {activeTab === 'stats' && (
          <motion.div key="stats" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            <div className="bg-slate-50 backdrop-blur-xl border border-slate-200 rounded-3xl p-6">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <span className="material-icons text-amber-400">trending_up</span>
                Ratios del Sistema
              </h2>
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-slate-900/50 rounded-2xl p-4 border border-slate-200 text-center">
          ...
          <motion.div key="users" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3 pl-12 pr-4 focus:outline-none focus:border-amber-500/50 transition-colors"
              />
            </div>

            <div className="space-y-3">
              {filteredUsers.map(u => (
                <div key={u.id} className="bg-slate-50 backdrop-blur-md border border-slate-200 rounded-2xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <img src={u.fotoUrl || '/placeholder-user.png'} alt={u.nick} className="w-10 h-10 rounded-full object-cover border border-slate-200" />
                    <div>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    <button 
                      ...
                      className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all ${
                        u.premium ? 'bg-amber-500/20 text-amber-500 border border-amber-500/30' : 'bg-slate-50 text-slate-400 border border-slate-200'
                      }`}
                    >
            ...
            {lastVisible && (
              <button 
                ...
                className="w-full py-3 bg-slate-50 hover:bg-slate-100 rounded-2xl text-xs font-bold text-slate-300 border border-slate-200 transition-all"
              >
        ...
        {activeTab === 'reports' && (
          <motion.div key="reports" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            {reports.map(report => (
              <div key={report.id} className="bg-slate-50 backdrop-blur-md border border-rose-500/10 rounded-2xl p-4 space-y-3">
...
function TabButton({ active, onClick, label, icon }: { active: boolean, onClick: () => void, label: string, icon: string }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold transition-all whitespace-nowrap ${active ? 'bg-amber-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400 border border-slate-200'}`}>
