...
export default function SetupProfilePage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      localStorage.setItem('darknights_city', city);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white p-6 pb-24 selection:bg-amber-500/30">
      <div className="w-full max-w-xl space-y-8">
        {/* Progress Bar */}
        <div className="flex gap-2">
          {[...Array(totalSteps)].map((_, i) => (
            <div key={i} className="flex-1 h-1.5 rounded-full bg-slate-50 overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: currentStep > i ? '100%' : '0%' }}
                className="h-full bg-gradient-to-r from-slate-700 to-blue-800"
              />
            </div>
          ))}
        </div>

        <div className="bg-slate-50 backdrop-blur-2xl border border-slate-200 rounded-[2.5rem] p-10 shadow-2xl space-y-10">
          <header className="text-center space-y-4">
            <div className="inline-block p-4 rounded-[1.5rem] bg-gradient-to-tr from-slate-700 to-blue-800 shadow-2xl mb-2">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                ...
                <div className="flex flex-col items-center space-y-6">
                  <div className="relative w-40 h-40 group">
                    <div className="w-full h-full rounded-full overflow-hidden border-4 border-slate-200 group-hover:border-amber-500 transition-all duration-500 shadow-2xl">
                      {imagePreview ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      placeholder="Ej: Marc"
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 text-white font-bold outline-none focus:bg-slate-100 transition-all"
                    />
                  </div>
                  <div className="space-y-3">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    ...
                    <select
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 text-white font-bold outline-none appearance-none"
                    >
                ...
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-slate-50 border border-slate-200 rounded-3xl p-5 text-white font-bold outline-none resize-none transition-all"
                  />
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-3">
                    ...
                    <input type="number" value={altura || ''} onChange={(e) => setAltura(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-white text-center font-bold" placeholder="180" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Peso</label>
                    <input type="number" value={peso || ''} onChange={(e) => setPeso(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-white text-center font-bold" placeholder="75" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Cuerpo</label>
                    <select value={complexion || ''} onChange={(e) => setComplexion(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-white font-bold text-xs appearance-none">
                      <option value="">--</option>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    {extraPhotosPreview.map((url, idx) => (
                      <div key={idx} className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-slate-50 border border-slate-200 shadow-xl">
                        <img src={url} alt={`Extra ${idx}`} className="w-full h-full object-cover" />
                      </div>
                    ))}
                    {extraPhotosPreview.length < 5 && (
                      <div className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-slate-50 border-2 border-dashed border-slate-200 hover:border-amber-500/50 transition-all flex items-center justify-center cursor-pointer">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    ...
                    {privatePhotosPreview.length < 5 && (
                      <div className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-slate-50 border-2 border-dashed border-yellow-500/20 hover:border-yellow-500/50 transition-all flex items-center justify-center cursor-pointer">
          ...
          <div className="flex gap-4 pt-4">
            {currentStep > 1 && (
              <button
                onClick={prevStep}
                className="flex-1 bg-slate-50 hover:bg-slate-100 text-white font-black py-6 rounded-[2rem] text-xs uppercase tracking-widest transition-all border border-slate-200"
              >
                Atrás
              </button>
            )}
            ...
            {currentStep < totalSteps ? (
              ...
              <button
                ...
                className={`flex-[2] bg-gradient-to-r from-slate-800 to-blue-900 text-white font-black py-6 rounded-[2rem] text-xs uppercase tracking-[0.3em] shadow-[0_20px_50px_rgba(245,158,11,0.3)] transition-all ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-95'}`}
              >
