...
export default function PremiumPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-slate-900 p-6 pb-24 relative overflow-hidden">
      {/* Background elements */}
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-amber-600/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      <main className="max-w-6xl mx-auto w-full space-y-8 relative z-10">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-slate-100 backdrop-blur-md p-1 rounded-full inline-flex border border-slate-200">
            <button 
            ...
            <button 
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-2 ${period === 'yearly' ? 'bg-gradient-to-r from-slate-700 to-blue-800 text-white shadow-lg' : 'text-white/70 hover:text-white'}`}
            >
              {t('premium.yearly')} <span className="bg-slate-100 text-xs px-2 py-0.5 rounded-full">-40%</span>
            </button>
          </div>
        </div>
        ...
        {(isPlus || isBlack) && !isPromoLifetime && (
          <div className="bg-slate-100 border border-slate-200 p-6 rounded-3xl flex justify-between items-center mb-8 backdrop-blur-sm">
            <div>
              <p className="text-slate-400 text-xs uppercase tracking-widest font-bold">{t('premium.current_plan')}</p>
              <h3 className={`text-2xl font-black ${isBlack ? 'text-transparent bg-clip-text bg-gradient-to-r from-slate-300 to-white' : 'text-amber-400'}`}>
                {isBlack ? 'BLACK' : 'PLUS'}
              </h3>
            </div>
            <button 
              ...
              className="bg-slate-100 hover:bg-slate-100 px-4 py-2 rounded-xl text-sm font-bold transition-all border border-slate-200"
            >
              {t('premium.manage')}
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          ...
          <motion.div 
            whileHover={{ y: -5 }}
            className={`bg-slate-50 backdrop-blur-xl border p-8 rounded-[2rem] flex flex-col relative overflow-hidden transition-all ${membershipTier === 'free' && !isPromoLifetime ? 'border-white/40 shadow-[0_0_20px_rgba(255,255,255,0.1)]' : 'border-slate-200 opacity-70'}`}
          >
            ...
            {membershipTier === 'free' && !isPromoLifetime && (
              <div className="mt-8">
                <div className="bg-slate-200 border border-blue-500/30 p-4 rounded-2xl relative overflow-hidden group">
                  <div className="absolute inset-0 bg-gradient-to-r from-blue-600/10 to-amber-600/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <h4 className="text-sm font-black text-blue-400 mb-1">Black 8h Pass</h4>
                  ...
                  {timeLeft ? (
                    <div className="bg-blue-500/20 text-blue-300 font-mono text-center py-2 rounded-xl text-xs font-bold border border-blue-500/30">
                      Activo: {timeLeft}
                    </div>
                  ) : (
                    <button
                      ...
                      className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 rounded-xl text-xs transition-all shadow-lg shadow-blue-500/20"
                    >
                      Comprar por 5€
                    </button>
                  )}
          ...
          <motion.div 
            whileHover={{ y: -5 }}
            className={`bg-slate-50 backdrop-blur-xl border p-8 rounded-[2rem] flex flex-col relative overflow-hidden transition-all ${isPlus ? 'border-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.2)]' : 'border-slate-200 hover:border-amber-500/50'}`}
          >
            <div className="space-y-2 mb-8">
              <h3 className="text-xl font-black uppercase tracking-widest text-amber-400">Plus</h3>
              <div className="flex items-baseline gap-1">
            ...
            <ul className="space-y-4 text-xs font-bold text-slate-300 flex-grow">
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">add_circle</span> <span className="text-white">Todo lo Free</span></li>
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">visibility</span> Ver quién te ha visitado</li>
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">visibility_off</span> Modo fantasma</li>
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">bolt</span> 10 pings al día</li>
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">rocket_launch</span> 1 boost gratis al mes</li>
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">tune</span> Filtros avanzados</li>
              <li className="flex items-center gap-3"><span className="material-icons text-amber-500 text-sm">favorite</span> Ver quién te dio like</li>
            </ul>
            <div className="mt-8">
              {!isPlus && !isBlack && !isPromoLifetime && (
                <button
              ...
              {isPlus && (
                <div className="text-center py-4 bg-amber-500/10 rounded-2xl border border-amber-500/30">
                  <span className="text-[10px] font-bold text-amber-400 uppercase">Tu plan actual</span>
                </div>
              )}
            </div>
          </motion.div>
          ...
          >
            ...
            <ul className="space-y-4 text-xs font-bold text-slate-300 flex-grow">
              ...
              <li className="flex items-center gap-3 font-black text-white bg-slate-100 p-2 rounded-xl border border-slate-200"><span className="material-icons text-sm">nightlife</span> Crear Chills y Afters</li>
