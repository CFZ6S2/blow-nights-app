      ...
      <main className="relative z-10 p-6 md:p-12 max-w-lg mx-auto space-y-6">
        <header className="pt-8 space-y-2">
          <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
            <span className="material-icons text-amber-400">qr_code_scanner</span>
            {t('scanner_title')}
          </h1>
          ...
          <div className="space-y-4">
            <div className="relative rounded-3xl overflow-hidden bg-black aspect-[4/3] flex items-center justify-center">
              {!scanning && !cameraError && (
                <button
                  onClick={startCamera}
                  className="px-6 py-3 bg-amber-600 text-white font-bold rounded-2xl flex items-center gap-2 z-10"
                >
              ...
              {scanning && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="w-48 h-48 border-2 border-amber-500 rounded-3xl animate-pulse" />
                </div>
              )}
              {cameraError && (
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-amber-500/50"
                />
                <button
                  onClick={() => validateToken(manualToken)}
                  disabled={!manualToken || validating}
                  className="px-6 py-3 rounded-2xl bg-amber-600 text-white text-[10px] font-black uppercase tracking-widest disabled:opacity-50"
                >
                  {validating ? '...' : 'Validar'}
                </button>
              </div>
        ...
        {mode === 'manual' && !result && (
          <div className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 p-6 rounded-3xl space-y-4">
              <label className="text-xs font-bold text-slate-400 block">Token de la entrada</label>
              <input
                type="text"
                value={manualToken}
                onChange={(e) => setManualToken(e.target.value)}
                placeholder="Pega aquí el token QR del cliente..."
                className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-4 py-4 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-amber-500/50 font-mono"
              />
              <button
                onClick={() => validateToken(manualToken)}
                disabled={!manualToken || validating}
                className="w-full py-4 rounded-2xl bg-amber-600 text-white text-xs font-black uppercase tracking-[0.2em] disabled:opacity-50 transition-all active:scale-95"
              >
        ...
        <AnimatePresence>
          {result && (
            ...
            >
              ...
              <button
                onClick={handleReset}
                className="w-full py-4 rounded-2xl bg-slate-100 text-white font-black uppercase tracking-[0.2em] text-xs hover:bg-slate-100 transition-all"
              >
