...
export default function CityManagerDashboard() {
  ...
  if (authLoading || dataLoading) return <div className="min-h-screen bg-white flex items-center justify-center"><div className="w-8 h-8 border-4 border-amber-500 border-t-transparent rounded-full animate-spin" /></div>;
  ...
  if (!cityData) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <button onClick={() => router.push('/')} className="mt-8 px-6 py-2 bg-slate-100 hover:bg-slate-100 rounded-xl text-white font-bold transition-colors">
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-amber-500/30 pb-20">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-gradient-to-br from-amber-900/40 to-blue-900/40 border border-amber-500/20 p-6 rounded-3xl relative overflow-hidden group shadow-[0_0_30px_-10px_rgba(59,130,246,0.2)]">
              <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                <span className="text-8xl font-black">€</span>
              </div>
              <p className="text-sm font-bold text-amber-300 mb-1">Tu Comisión Neta (50%)</p>
              <p className="text-4xl font-black text-white">{metrics.managerPayoutDue.toFixed(2)} €</p>
              <div className="mt-4 pt-4 border-t border-amber-500/20 flex items-center text-xs text-amber-200/50 font-medium">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              ...
              className="text-xs bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20 px-4 py-2 rounded-lg font-bold transition-colors flex items-center gap-2"
            >
                ...
                {venues.map(v => (
                  <div key={v.id} className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-800/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${v.subscriptionTier === 'ticketing' ? 'bg-blue-500/10 text-blue-400' : v.subscriptionTier === 'promo' ? 'bg-amber-500/10 text-amber-400' : v.subscriptionTier === 'basico' ? 'bg-blue-500/10 text-blue-400' : 'bg-slate-800 text-slate-400'}`}>
