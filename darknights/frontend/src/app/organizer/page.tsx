...
function EventPromoters({ eventId }: { eventId: string }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-200 space-y-4">
      <h4 className="text-sm font-bold text-blue-400">{t('organizer_page.rrpp_team')}</h4>
      ...
      <div className="flex gap-2">
        <input 
          ...
          className="flex-1 bg-slate-200 border border-slate-200 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
        />
        <button onClick={handleAddPromoter} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold">
      ...
      {loading ? (
        ...
        <div className="space-y-3 mt-4">
          {promoters.map(p => {
            ...
            return (
              <div key={p.id} className={`flex items-center justify-between p-3 rounded-xl border ${p.is_closed ? 'bg-slate-200 border-yellow-500/20' : 'bg-slate-100 border-slate-200'}`}>
                <div>
                  ...
                  {!p.is_closed && (
                    <button onClick={() => copyLink(p.access_token)} className="text-[10px] text-blue-400 hover:text-blue-300 flex items-center mt-1">
...
function EventMagicLink({ eventId }: { eventId: string }) {
  ...
  return (
    <div className="flex items-center justify-between bg-slate-200 p-4 rounded-xl border border-blue-500/20">
      <div>
        <div className="text-[10px] text-blue-400 font-bold uppercase tracking-widest">Enlace de Portero (Scanner)</div>
...
export default function OrganizerDashboard() {
  ...
  if (authLoading || loading) return <div className="min-h-screen bg-white flex justify-center items-center"><div className="w-8 h-8 border-2 border-blue-500 rounded-full animate-spin border-t-transparent" /></div>;
    ...
    <div className="min-h-screen bg-white text-slate-900 p-6 md:p-12">
      <main className="relative z-10 p-6 md:p-12 max-w-4xl mx-auto space-y-8">
        <header className="pt-8">
          <div className="inline-block px-3 py-1 bg-blue-900/30 text-blue-400 rounded-full text-[10px] font-black tracking-widest uppercase mb-4 border border-blue-500/20">
        ...
        {events.length > 0 && !isCreating && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-amber-900/40 to-slate-900/40 border border-amber-500/30 p-6 rounded-3xl relative">
              <div className="text-[10px] font-black uppercase tracking-widest text-amber-400 mb-1">Créditos QR</div>
              <div className="text-3xl font-black text-white">{(profile as any)?.qr_quota || 0}</div>
              <button 
                onClick={() => setBuyQRModal(true)}
                className="absolute top-1/2 -translate-y-1/2 right-6 bg-amber-600 hover:bg-amber-500 text-white px-4 py-2 rounded-xl text-xs font-bold"
              >
                Recargar
              </button>
            </div>
            <div className="bg-gradient-to-br from-blue-900/40 to-slate-900/40 border border-blue-500/30 p-6 rounded-3xl">
              <div className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-1">{t('organizer_page.total_revenue')}</div>
              <div className="text-3xl font-black text-white">{totalRevenue.toFixed(2)}€</div>
            </div>
            <div className="bg-gradient-to-br from-blue-900/40 to-slate-900/40 border border-blue-500/30 p-6 rounded-3xl">
              <div className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-1">{t('organizer_page.total_attendees')}</div>
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="px-6 py-3 bg-blue-600 text-white rounded-full text-xs font-bold uppercase tracking-widest hover:bg-blue-500 transition-colors"
          >
        ...
        {isCreating && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-900 border border-blue-500/30 rounded-3xl p-8 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-200 border border-slate-200 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500"
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700"
                />
                {flyerPreview && (
              ...
              <div className="pt-4 border-t border-slate-200 space-y-4">
                <div className="flex justify-between items-center">
                  <h4 className="font-bold text-blue-400">Tipos de Entrada (Tiers)</h4>
                  <button onClick={addTier} className="text-xs text-blue-400 border border-blue-400 px-3 py-1 rounded-full hover:bg-blue-400/10">+ Añadir</button>
                </div>
                
                {tiers.map((tier, index) => (
                  <div key={tier.id} className="bg-slate-200 p-4 rounded-2xl border border-slate-200 space-y-3 relative">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div>
                        <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Nombre</label>
                        <input type="text" value={tier.name} onChange={e => updateTier(index, 'name', e.target.value)} className="w-full bg-transparent border-b border-slate-200 focus:border-blue-500 pb-1 text-sm outline-none" placeholder="Ej. General" />
                      </div>
                      <div>
                        <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Precio (€)</label>
                        <input type="number" value={tier.price} onChange={e => updateTier(index, 'price', parseFloat(e.target.value))} className="w-full bg-transparent border-b border-slate-200 focus:border-blue-500 pb-1 text-sm outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Cupo (Aforo)</label>
                        <input type="number" value={tier.quota} onChange={e => updateTier(index, 'quota', parseInt(e.target.value))} className="w-full bg-transparent border-b border-slate-200 focus:border-blue-500 pb-1 text-sm outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Perks (Opcional)</label>
                        <input type="text" value={tier.perks} onChange={e => updateTier(index, 'perks', e.target.value)} className="w-full bg-transparent border-b border-slate-200 focus:border-blue-500 pb-1 text-sm outline-none" placeholder="1 Copa..." />
                      </div>
                    </div>
                    {tiers.length > 1 && (
              ...
              <button
                ...
                disabled={saving}
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest transition-colors mt-6 shadow-[0_0_20px_rgba(37,99,235,0.3)] disabled:opacity-50"
              >
        ...
        <div className="space-y-4">
          ...
          {events.map(evt => (
            <div key={evt.id} className="bg-slate-50 border border-slate-200 rounded-3xl p-6 flex flex-col md:flex-row gap-6">
              {evt.banner_url ? (
              ...
              <div className="flex-1 space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <span className="px-2 py-1 bg-blue-500/20 text-blue-400 text-[10px] font-bold uppercase rounded-md mb-2 inline-block">
                      {evt.cityId}
                    </span>
                  ...
                  <div className="text-right">
                    <div className="text-3xl font-black text-blue-400">{evt.stats?.total_sold || 0}</div>
                ...
                <div className="pt-4 mt-4 border-t border-slate-200 flex gap-2">
                  <button 
                    ...
                    className="flex-1 bg-amber-600 hover:bg-amber-500 text-white py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
                  >
        ...
        {buyQRModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur-sm p-4">
            <div className="bg-slate-900 border border-slate-200 rounded-3xl p-6 max-w-sm w-full space-y-6">
              <div className="flex justify-between items-center">
              ...
              <div className="space-y-3">
                {[25, 50, 100, 500].map(qty => (
                  <button 
                    ...
                    disabled={buyingQRs}
                    className="w-full flex justify-between items-center bg-slate-50 hover:bg-slate-100 p-4 rounded-2xl border border-slate-200 disabled:opacity-50"
                  >
                    <span className="font-bold">{qty} QRs</span>
                    <span className="text-blue-400 font-bold">{qty} €</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur-sm p-4">
            <div className="bg-slate-900 border border-slate-200 rounded-3xl p-6 max-w-sm w-full space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-200 border border-slate-200 rounded-xl px-4 py-3 text-white focus:outline-none"
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-200 border border-slate-200 rounded-xl px-4 py-3 text-white focus:outline-none appearance-none"
                  >
              ...
              <button
                ...
                className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl font-black uppercase tracking-widest disabled:opacity-50"
              >
