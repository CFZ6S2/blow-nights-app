    ...
    >
      ...
      >
        {u.online && (
          <div
            className={`absolute inset-0 rounded-full blur-xl -z-10 animate-marker-pulse ${
              isBoosted ? 'bg-gradient-to-r from-yellow-400 to-amber-500' : 'bg-amber-500/40'
            }`}
          />
        )}
        ...
        <div className={`w-12 h-12 rounded-full border-2 ${
          ...
          : u.premium ? 'border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 'border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.4)]'
        ...
        {u.mood && (
          <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-slate-100 backdrop-blur-md border border-slate-200 rounded-full w-8 h-8 flex items-center justify-center text-lg shadow-xl animate-mood-float">
            {u.mood}
          </div>
        )}
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 hidden group-hover:block z-50 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <motion.div 
            ...
            className="bg-slate-900/90 backdrop-blur-xl border border-slate-200 p-4 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] min-w-[160px]"
          >
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 rounded-lg bg-slate-50 text-[10px] text-slate-300 font-bold capitalize">{u.rol}</span>
              <span className="px-2 py-1 rounded-lg bg-amber-500/10 text-[10px] text-amber-400 font-bold capitalize">{u.intencion}</span>
            </div>
          ...
          <div className="w-3 h-3 bg-slate-900/90 rotate-45 absolute -bottom-1.5 left-1/2 -translate-x-1/2 border-r border-b border-slate-200"></div>
        </div>
      </motion.div>
    </Marker>
  );
...
const VenueMarker = memo(({ v, onClick, index }: { v: any, onClick: (id: string) => void, index: number }) => {
  ...
  // Style mapping
  let bgClass = 'bg-slate-900 border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.4)]';
  let iconName = 'place';
  let gradientClass = 'bg-blue-500/20';
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block z-50 animate-in fade-in slide-in-from-bottom-2 duration-300 pointer-events-none">
          <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-200 px-4 py-2 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] whitespace-nowrap flex flex-col items-center">
...
const ChillMarker = memo(({ c, onClick, index }: { c: Chill, onClick: (id: string) => void, index: number }) => {
  ...
  const colorClass = isCommercial ? 'amber' : 'amber';
  ...
  return (
    <Marker latitude={c.approx_lat} longitude={c.approx_lng} anchor="center">
      <motion.div
        ...
        className="relative group cursor-pointer"
      >
        <div className={`absolute inset-0 rounded-full ${isCommercial ? 'bg-amber-500/30' : 'bg-amber-500/20'} blur-xl -z-10 animate-marker-pulse`} />
        <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${isCommercial ? 'from-amber-400 to-orange-500 border-amber-300 shadow-[0_0_20px_rgba(245,158,11,0.5)]' : 'from-amber-500 to-red-700 border-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.5)]'} border-2 flex items-center justify-center group-hover:scale-110 transition-transform`}>
          <span className="material-icons text-white text-xl">{isCommercial ? 'vpn_key' : 'local_fire_department'}</span>
        </div>
        <div className={`absolute -bottom-1 -right-1 bg-black border ${isCommercial ? 'border-amber-500/50 text-amber-400' : 'border-amber-500/50 text-amber-400'} rounded-full px-1.5 py-0.5 text-[8px] font-black`}>
          {spots > 0 ? spots : '0'}
        </div>
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block z-50 pointer-events-none">
          <div className={`bg-slate-900/90 backdrop-blur-xl border ${isCommercial ? 'border-amber-500/30' : 'border-amber-500/30'} px-4 py-2 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] whitespace-nowrap flex flex-col items-center`}>
            <p className="font-black text-sm text-white">{c.title}</p>
            <p className={`text-[9px] font-bold ${isCommercial ? 'text-amber-400' : 'text-amber-400'}`}>
              {isCommercial ? 'After Privado' : 'Chill Social'} &middot; {spots > 0 ? `${spots} plazas` : 'Lleno'}
            </p>
          </div>
          <div className={`w-3 h-3 bg-slate-900/90 rotate-45 absolute -bottom-1.5 left-1/2 -translate-x-1/2 border-r border-b ${isCommercial ? 'border-amber-500/30' : 'border-amber-500/30'}`}></div>
        </div>
      </motion.div>
    </Marker>
  );
...
export default function MainMap() {
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      <div className="relative w-full h-[65svh] md:h-[65vh] rounded-[2.5rem] overflow-hidden border border-slate-200 shadow-2xl bg-white">
      <motion.div 
        animate={{ 
          ...
          <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center z-30">
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center text-amber-500">
                <span className="material-icons text-4xl shadow-2xl drop-shadow-[0_0_15px_rgba(245,158,11,0.8)]">
                  {newPinType === 'cruising_outdoor' ? 'park' : 'local_fire_department'}
                </span>
              </div>
              <div className="absolute top-full left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-amber-500 rounded-full shadow-[0_0_10px_#f59e0b]"></div>
            </motion.div>
            ...
            <motion.div 
              ...
              className="absolute bottom-8 w-[90%] max-w-sm bg-slate-900/95 backdrop-blur-2xl border border-slate-200 p-5 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] pointer-events-auto"
            >
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button 
                    ...
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPinType === 'sauna' ? 'bg-red-600 text-white shadow-lg shadow-red-500/20' : 'bg-slate-200 text-slate-400'}`}
                  >
                    Sauna ♨️
                  </button>
                  <button 
                    onClick={() => setNewPinType('bar')} 
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPinType === 'bar' ? 'bg-orange-600 text-white shadow-lg shadow-orange-500/20' : 'bg-slate-200 text-slate-400'}`}
                  >
                    Bar 🍸
                  </button>
                  <button 
                    onClick={() => setNewPinType('club')} 
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPinType === 'club' ? 'bg-cyan-600 text-white shadow-lg shadow-cyan-500/20' : 'bg-slate-200 text-slate-400'}`}
                  >
                    Discoteca 🪩
                  </button>
                </div>
                <input 
                  ...
                  className="w-full bg-slate-200 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-amber-500 outline-none text-white"
                />
                <button 
          ...
          <div className="absolute bottom-4 left-4 right-4 z-20 flex items-end justify-between">
            ...
            <motion.button
              ...
              className={`w-14 h-14 rounded-2xl backdrop-blur-xl border flex items-center justify-center shadow-lg transition-all ${
                isAvailable
                  ? 'bg-amber-600/80 border-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.4)]'
                  : 'bg-slate-900/60 border-slate-200'
              }`}
            >
            ...
            <div className="flex items-center gap-2">
              {profile?.premium && (
                <motion.button
                  ...
                  className={`w-12 h-12 rounded-2xl backdrop-blur-xl text-white border flex items-center justify-center shadow-lg transition-all ${
                    isAddingPin ? 'bg-amber-600 border-amber-400' : 'bg-slate-900/60 border-slate-200 hover:bg-slate-100'
                  }`}
                >
              ...
              <motion.button
                ...
                className="w-12 h-12 rounded-2xl bg-slate-900/60 backdrop-blur-xl text-white border border-slate-200 flex items-center justify-center shadow-lg hover:bg-slate-100 transition-all"
              >
                <span className="material-icons text-xl text-amber-400">my_location</span>
              </motion.button>

              <motion.button
                whileTap={{ scale: 0.9 }}
                onClick={() => { triggerHaptic(10); setFiltersOpen(true); }}
                className="relative w-12 h-12 rounded-2xl bg-slate-900/60 backdrop-blur-xl text-white border border-slate-200 flex items-center justify-center shadow-lg hover:bg-slate-100 transition-all"
              >
                <span className="material-icons text-xl text-amber-400">tune</span>
                {(roleFilter || intentionFilter || onlyPremium) && (
                  <div className="absolute top-2 right-2 w-3 h-3 rounded-full bg-amber-500 border-2 border-slate-900 animate-pulse shadow-[0_0_8px_#f59e0b]"></div>
                )}
              </motion.button>
            </div>
          </div>
      ...
      <AnimatePresence>
        {filtersOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-slate-300 backdrop-blur-md flex items-end justify-center px-4 pb-4"
          >
            <motion.div 
              initial={{ scale: 0.9, opacity: 0, y: 20 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              className="w-full max-w-lg bg-slate-900 rounded-[2.5rem] p-6 space-y-6 shadow-[0_-20px_80px_rgba(0,0,0,0.8)] border border-slate-200 max-h-[85svh] overflow-y-auto"
            ...
            >
              <div className="flex justify-between items-center pb-2">
                ...
                <button 
                  ...
                  className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center hover:bg-slate-100 transition-all"
                >
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex items-center gap-3 focus-within:border-amber-500 transition-colors">
                ...
                <div className="flex bg-slate-200 border border-slate-200 rounded-xl p-1">
                  <button 
                    onClick={() => setRadarFilter(prev => ({ ...prev, mode: 'places' }))}
                    className={`flex-1 py-2.5 rounded-lg text-[10px] uppercase font-black transition-all flex items-center justify-center gap-1 ${radarFilter.mode === 'places' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                  >
                    <span>📍</span> {t('map.mode_places', 'Lugares')}
                  </button>
                  <button 
                    onClick={() => setRadarFilter(prev => ({ ...prev, mode: 'users' }))}
                    className={`flex-1 py-2.5 rounded-lg text-[10px] uppercase font-black transition-all flex items-center justify-center gap-1 ${radarFilter.mode === 'users' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                  >
              ...
              {radarFilter.mode === 'places' ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {PLACE_CATEGORY_KEYS.map(cat => (
                      <button 
                        ...
                        className={`py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${radarFilter.type === cat.id ? "bg-amber-500 text-white shadow-xl shadow-amber-500/20" : "bg-slate-50 text-slate-400 border border-slate-200 hover:bg-slate-100"}`}
                      >
                ...
                <div className="flex justify-between items-end">
                  ...
                  <p className="text-amber-400 font-black text-lg">{ageRange[0]} - {ageRange[1]}</p>
                </div>
                <div className="space-y-4">
                  <input type="range" min={18} max={99} value={ageRange[0]} onChange={(e) => setAgeRange([Number(e.target.value), ageRange[1]])} className="w-full h-1.5 bg-slate-50 rounded-full appearance-none accent-amber-500 cursor-pointer" />
                  <input type="range" min={18} max={99} value={ageRange[1]} onChange={(e) => setAgeRange([ageRange[0], Number(e.target.value)])} className="w-full h-1.5 bg-slate-50 rounded-full appearance-none accent-amber-500 cursor-pointer" />
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  ...
                  <p className="text-amber-400 font-black text-lg">{distanceFilter} km</p>
                </div>
                <div className="space-y-4">
                  <input 
                    type="range" 
                    ...
                    className="w-full h-1.5 bg-slate-50 rounded-full appearance-none accent-amber-500 cursor-pointer" 
                  />
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {["activo", "pasivo", "versátil"].map((r) => (
                      <button key={r} onClick={() => { triggerHaptic(10); setRoleFilter(roleFilter === r ? null : r); }} className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${roleFilter === r ? "bg-amber-500 text-white shadow-xl shadow-amber-500/20" : "bg-slate-50 text-slate-400 border border-slate-200"}`}>{r}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {["conocer", "quedar"].map((i) => (
                      <button key={i} onClick={() => { triggerHaptic(10); setIntentionFilter(intentionFilter === i ? null : i); }} className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${intentionFilter === i ? "bg-amber-500 text-white shadow-xl shadow-amber-500/20" : "bg-slate-50 text-slate-400 border border-slate-200"}`}>{i}</button>
                    ))}
                  </div>
                </div>
              </div>
