...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-white/90 backdrop-blur-md border-t border-slate-200 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_20px_rgba(0,0,0,0.06)] translate-z-0">
              ...
              >
                <span
                  className={`material-icons text-2xl block transition-colors duration-300 ${
                    active ? "text-amber-600 drop-shadow-[0_0_8px_rgba(217,119,6,0.3)]" : "text-slate-400"
                  }`}
                >
                  {item.icon}
                </span>
                ...
                {active && (
                  <motion.div
                    ...
                    className="absolute -top-4 left-1/2 -translate-x-1/2 h-1 w-6 rounded-full bg-amber-500/80 blur-[2px]"
                    transition={{ type: "spring", stiffness: 250, damping: 20 }}
                  />
                )}
              </motion.div>

              <span className={`text-[10px] font-black uppercase tracking-tighter transition-colors duration-300 ${active ? "text-amber-600" : "text-slate-400"}`}>
                {item.label}
              </span>
            </Link>
          );
