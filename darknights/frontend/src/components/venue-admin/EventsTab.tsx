...
function EventMagicLink({ venueId, eventId }: { venueId: string; eventId: string }) {
  ...
  return (
    <div className="flex items-center justify-between bg-slate-200 p-3 rounded-xl border border-blue-500/20">
      <div>
        <div className="text-[10px] text-blue-400 font-bold uppercase tracking-widest">Enlace de Portero (Magic Link)</div>
        <div className="text-xs text-slate-400 font-mono mt-1">/door/{venueId}/{eventId}?token={token.substring(0,8)}...</div>
      </div>
      <button 
        onClick={copyMagicLink}
        className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center hover:scale-105 transition-transform"
