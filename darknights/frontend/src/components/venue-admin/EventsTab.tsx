...
function EventPromoters({ venueId, eventId }: { venueId: string; eventId: string }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-200 space-y-4">
      <h4 className="text-sm font-bold text-amber-400">Equipo RRPP</h4>
      ...
      <div className="flex gap-2">
        <input 
          ...
          className="flex-1 bg-slate-200 border border-slate-200 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500"
        />
        <button onClick={handleAddPromoter} className="bg-amber-600 text-white px-4 py-2 rounded-xl text-sm font-bold">
          Añadir
        </button>
      </div>
            ...
            return (
              <div key={p.id} className={`flex items-center justify-between p-3 rounded-xl border ${p.is_closed ? 'bg-slate-200 border-yellow-500/20' : 'bg-slate-100 border-slate-200'}`}>
                <div>
                  ...
                  {!p.is_closed && (
                    <button onClick={() => copyLink(p.access_token)} className="text-[10px] text-amber-400 hover:text-amber-300 flex items-center mt-1">
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="px-4 py-2 bg-amber-600 text-white rounded-full text-xs font-bold uppercase tracking-widest"
        >
      ...
      {isCreating && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-900 border border-slate-200 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500"
            ...
            <div>
              ...
              <input 
                ...
                className="w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-amber-600 file:text-white hover:file:bg-amber-700"
              />
              {flyerPreview && (
            ...
            <div className="pt-4 border-t border-slate-200 space-y-4">
              <div className="flex justify-between items-center">
                <h4 className="font-bold text-amber-400">Tipos de Entrada (Tiers)</h4>
                <button onClick={addTier} className="text-xs text-amber-400 border border-amber-400 px-3 py-1 rounded-full">+ Añadir</button>
              </div>
              
              {tiers.map((tier, index) => (
                <div key={tier.id} className="bg-slate-900 p-4 rounded-2xl border border-slate-200 space-y-3 relative">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Nombre</label>
                      <input type="text" value={tier.name} onChange={e => updateTier(index, 'name', e.target.value)} className="w-full bg-transparent border-b border-slate-200 focus:border-amber-500 pb-1 text-sm outline-none" placeholder="Ej. General" />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Precio (€)</label>
                      <input type="number" value={tier.price} onChange={e => updateTier(index, 'price', parseFloat(e.target.value))} className="w-full bg-transparent border-b border-slate-200 focus:border-amber-500 pb-1 text-sm outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Cupo (Aforo)</label>
                      <input type="number" value={tier.quota} onChange={e => updateTier(index, 'quota', parseInt(e.target.value))} className="w-full bg-transparent border-b border-slate-200 focus:border-amber-500 pb-1 text-sm outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Perks (Ej. 1 Copa)</label>
                      <input type="text" value={tier.perks} onChange={e => updateTier(index, 'perks', e.target.value)} className="w-full bg-transparent border-b border-slate-200 focus:border-amber-500 pb-1 text-sm outline-none" />
                    </div>
                  </div>
                  {tiers.length > 1 && (
            ...
            <button
              ...
              disabled={saving}
              className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl font-black uppercase tracking-widest transition-colors mt-6"
            >
      ...
      <div className="space-y-4">
        ...
        {events.map(evt => (
          <div key={evt.id} className="bg-slate-50 border border-slate-200 rounded-3xl p-5 flex flex-col md:flex-row gap-5">
            {evt.banner_url ? (
            ...
            <div className="flex-1 space-y-2">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <div className="text-2xl font-black text-amber-400">{evt.stats?.total_sold || 0}</div>
                  <div className="text-[9px] uppercase tracking-widest text-slate-500">{t('sold_tickets')}</div>
                </div>
              </div>

              <div className="pt-3 border-t border-slate-200">
                <div className="flex items-center justify-between bg-slate-200 p-3 rounded-xl border border-amber-500/20">
                  <div>
                    <div className="text-[10px] text-amber-400 font-bold uppercase tracking-widest">Enlace de Portero (Magic Link)</div>
                  ...
                  <button 
                    ...
                    className="w-10 h-10 bg-amber-600 rounded-full flex items-center justify-center hover:scale-105 transition-transform"
