...
export default function CreateChillModal({
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-white/90 backdrop-blur-sm"
      onClick={onClose}
    >
      <motion.div
        initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }}
        onClick={(e) => e.stopPropagation()}
        className="bg-slate-900 border border-slate-200 w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 max-h-[90vh] overflow-y-auto"
      >
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={300}
              rows={3}
              className="w-full bg-black border border-slate-200 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500 resize-none"
            />
          </div>

          <div>
            <label className="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest">
              {t('chill_modal.address_label')} <span className="text-amber-400">{t('chill_modal.address_private')}</span>
            </label>
            <input
              type="text"
              value={address}
          ...
          <button
            ...
            disabled={submitting}
            className="w-full py-5 bg-amber-600 hover:bg-amber-500 disabled:bg-slate-800 disabled:text-slate-600 rounded-2xl font-black text-lg shadow-[0_0_40px_-10px_rgba(245,158,11,0.5)] transition-all active:scale-95"
          >
