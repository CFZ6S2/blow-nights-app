...
export default function PWAInstallPrompt() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="bg-slate-900/90 backdrop-blur-2xl border border-slate-200 rounded-[2.5rem] p-6 shadow-2xl flex items-center gap-6">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-slate-800 to-blue-900 flex items-center justify-center shrink-0 shadow-lg">
          ...
          <div className="flex flex-col gap-2">
            ...
            <button 
              ...
              className="px-6 py-3 rounded-full bg-slate-50 text-slate-400 font-black text-[10px] uppercase tracking-widest border border-slate-200 hover:bg-slate-100 transition-all"
            >
              {t('pwa.close_btn')}
            </button>
          </div>
        ...
        {platform === 'ios' && (
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-slate-900 rotate-45 border-r border-b border-slate-200" />
        )}
      </motion.div>
    </AnimatePresence>
  );
