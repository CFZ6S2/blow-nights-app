...
export default function MatchOverlay({ matchData, onClose }: { matchData: any, onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[250] bg-white/98 backdrop-blur-[50px] flex flex-col items-center justify-center p-8 text-center overflow-hidden"
    >
      ...
      <motion.div 
        ...
        className={`absolute w-[600px] h-[600px] rounded-full blur-[150px] -z-20 ${isSuper ? 'bg-yellow-500/30' : 'bg-amber-600/30'}`}
      />
      ...
      >
          ...
          {[...Array(40)].map((_, i) => (
            <motion.div
              ...
              className={`absolute w-1.5 h-1.5 rounded-full ${
                i % 4 === 0 ? (isSuper ? 'bg-yellow-400' : 'bg-amber-500') : 
                i % 4 === 1 ? 'bg-white' : 
                i % 4 === 2 ? (isSuper ? 'bg-orange-500' : 'bg-blue-500') :
                'bg-cyan-400'
        ...
        >
          <h1 className={`text-7xl font-[1000] italic tracking-tighter mb-2 leading-[0.8] ${
            isSuper ? 'text-transparent bg-clip-text bg-gradient-to-b from-yellow-200 via-yellow-400 to-yellow-700' : 'text-transparent bg-clip-text bg-gradient-to-b from-white via-amber-400 to-amber-600'
          } drop-shadow-[0_10px_30px_rgba(245,158,11,0.5)]`}>
        ...
        <div className="flex items-center justify-center mb-24 relative">
          ...
          <motion.div 
            ...
            className={`absolute w-48 h-48 rounded-full border-2 ${isSuper ? 'border-yellow-500' : 'border-amber-500'}`}
          />
          ...
          <motion.div
            ...
            className={`absolute z-30 w-24 h-24 rounded-full flex items-center justify-center shadow-[0_0_50px_rgba(0,0,0,0.5)] border-[6px] border-slate-950 ${
              isSuper ? 'bg-gradient-to-br from-yellow-400 to-orange-600' : 'bg-gradient-to-br from-amber-400 to-red-600'
            }`}
          >
            <motion.span 
