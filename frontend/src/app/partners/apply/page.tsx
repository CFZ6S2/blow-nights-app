...
export default function PartnerApplicationPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white selection:bg-purple-500/30 font-sans p-6">
      <div className="max-w-2xl mx-auto pt-12">
        ...
        {isSuccess ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-slate-900 border border-green-500/30 rounded-3xl p-12 text-center shadow-2xl">
            ...
            <h1 className="text-3xl font-black mb-4">Solicitud Recibida</h1>
            <p className="text-slate-400 mb-8 max-w-md mx-auto">
              Hemos recibido tu solicitud para ser City Manager. Nuestro equipo de expansión la revisará y se pondrá en contacto contigo muy pronto.
            </p>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
            <h1 className="text-3xl font-black mb-2">Solicita tu Territorio</h1>
            <p className="text-slate-400 mb-8 text-sm">Rellena este formulario para aplicar por la exclusividad de tu ciudad. El canon de City Manager es de 5.000 € y se abona tras la aprobación de tu perfil.</p>

            <form onSubmit={handleSubmit} className="space-y-6">
              
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Ciudad a reclamar</label>
                <select 
                  required
                  value={selectedCity}
                ...
                >
                  ...
                  {cities.map(c => (
                    <option key={c.id} value={c.id}>{c.name || c.id} (Oportunidad Activa)</option>
                  ))}
                  <option value="other">Otra ciudad (Especificar en experiencia)</option>
                </select>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input 
                  ...
                  placeholder="Ej: Carlos Santamaría"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Teléfono (WhatsApp)</label>
                <input 
                  required
                  type="tel" 
                  value={phone}
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Experiencia en Ocio Nocturno</label>
                <textarea 
                  required
                  value={experience}
                  onChange={(e) => setExperience(e.target.value)}
                  placeholder="Cuéntanos brevemente sobre tu red de contactos, locales con los que trabajas o tu experiencia como RRPP/Promotor..."
                  rows={4}
      ...
      <a 
        href="https://wa.me/34600000000?text=Hola,%20me%20gustar%C3%ADa%20solicitar%20la%20franquicia%20de%20City%20Manager%20para%20mi%20ciudad."
        target="_blank"
