...
function TicketViewInner() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white flex justify-center p-4 pb-12">
      <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl space-y-4 pb-6">
        ...
        <div className="relative h-48 w-full bg-slate-800">
          {ticket.flyerUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-fuchsia-900 via-purple-950 to-slate-950 flex flex-col items-center justify-center text-fuchsia-400/40">
              <span className="text-4xl mb-1">🪩</span>
              <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Blow Nights Pass</span>
            </div>
          )}
        ...
        <div className="p-5 pt-1 text-center space-y-4">
          <div className="inline-block p-3 bg-white rounded-2xl shadow-inner shadow-black/20">
            <QRCode 
              value={`https://blownights.com/scan?ticket=${ticket.qrToken}`}
              size={170}
              level="M"
            />
          </div>
