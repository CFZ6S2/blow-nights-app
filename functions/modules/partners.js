...
exports.requestPartnerAccess = onCall({ enforceAppCheck: true }, async (request) => {
  const { data } = request;
...
exports.approvePartnerAccess = onCall({ enforceAppCheck: true }, async (request) => {
  ...
  if (action === 'approve') {
    ...
    await appDoc.ref.update({
      ...
      reviewedAt: admin.firestore.FieldValue.serverTimestamp(),
      reviewedBy: auth.uid,
      uid: targetUid // Link application to created user
    });
