 ...
 */
exports.createUserMembershipCheckout = onCall({ enforceAppCheck: false, secrets: ["STRIPE_SECRET_KEY"] }, async (request) => {
 ...
 */
exports.createBlack8hCheckout = onCall({ enforceAppCheck: false, secrets: ["STRIPE_SECRET_KEY"] }, async (request) => {
  const { data, auth } = request;
  if (!auth) throw new HttpsError('unauthenticated', 'Login requerido.');
  const { origin } = data;
  const stripe = getStripe();
