        ...
        <main className="relative z-10 p-6 md:p-12 max-w-2xl mx-auto">
          <header className="pt-8 space-y-2 mb-8">
            <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
              <span className="material-icons text-slate-800">storefront</span>
              {t('venue_admin_title')}
            </h1>
          </header>
      ...
      <main className="relative z-10 p-6 md:p-12 max-w-2xl mx-auto space-y-6">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar">
              {myVenues.map((v) => (
                ...
                >
                  {v.name}
                  {v.ownerId === 'community' ? (
                    <span className="bg-slate-700/50 text-slate-600 px-1.5 py-0.5 rounded-[4px] text-[8px]">Comunitario</span>
                  ) : (
            ...
            <div className="flex items-center gap-2">
              {selectedVenue?.subscriptionTier && !isAdmin && (
                <button
                  ...
                  className="px-4 py-3 rounded-2xl border border-white/10 hover:bg-white/5 text-slate-900 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-colors"
                >
              ...
              {(selectedVenue?.subscriptionTier === 'ticketing' || isAdmin) && (
                <Link
                  href="/scanner"
                  className="px-4 py-3 rounded-2xl bg-slate-900 text-slate-900 text-[10px] font-black uppercase tracking-widest flex items-center gap-2"
                >
        ...
        {selectedVenue && !selectedVenue.subscriptionTier && selectedVenue.ownerId !== 'community' && !isAdmin && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white border border-slate-300 p-4 rounded-xl flex flex-col items-center text-center">
              <h3 className="font-black text-lg">Básico</h3>
              <p className="text-xl font-black text-slate-800">30€<span className="text-sm text-slate-500">/mes</span></p>
              <p className="text-xs text-slate-500 mt-2 mb-4">Presencia en el mapa y configuración básica.</p>
            ...
            <div className="bg-gradient-to-br from-fuchsia-900/40 to-indigo-900/40 border border-slate-700/50 p-4 rounded-xl flex flex-col items-center text-center">
              <h3 className="font-black text-lg">Promo</h3>
              <p className="text-xl font-black text-slate-800">60€<span className="text-sm text-slate-500">/mes</span></p>
              <p className="text-xs text-slate-500 mt-2 mb-4">Envía notificaciones push promocionales a usuarios cercanos.</p>
              <button onClick={() => handleSubscription('promo')} disabled={checkingOut} className="w-full mt-auto py-2 bg-slate-900 hover:bg-fuchsia-500 rounded-lg text-sm font-bold shadow-[0_0_15px_rgba(217,70,239,0.5)]">Activar</button>
            </div>
            <div className="bg-white border border-slate-300 p-4 rounded-xl flex flex-col items-center text-center">
              <h3 className="font-black text-lg">Ticketing</h3>
              <p className="text-xl font-black text-slate-800">100€<span className="text-sm text-slate-500">/mes</span></p>
              <p className="text-xs text-slate-500 mt-2 mb-4">Gestión de eventos, RRPP y escáner de entradas.</p>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            <button onClick={() => setTab('overview')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'overview' ? 'bg-slate-900 border-slate-700 text-slate-900' : 'bg-white/5 border-white/5 text-slate-500'}`}>{t('tab_overview')}</button>
            <button onClick={() => setTab('config')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'config' ? 'bg-slate-900 border-slate-700 text-slate-900' : 'bg-white/5 border-white/5 text-slate-500'}`}>{t('tab_config')}</button>
            <button onClick={() => setTab('catalog')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'catalog' ? 'bg-slate-900 border-slate-700 text-slate-900' : 'bg-white/5 border-white/5 text-slate-500'}`}>{t('tab_catalog')}</button>
            <button onClick={() => setTab('events')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'events' ? 'bg-slate-900 border-slate-700 text-slate-900' : 'bg-white/5 border-white/5 text-slate-500'}`}>{t('tab_events')}</button>
            <button onClick={() => setTab('tickets')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'tickets' ? 'bg-slate-900 border-slate-700 text-slate-900' : 'bg-white/5 border-white/5 text-slate-500'}`}>{t('tab_ticketing')}</button>
            <button onClick={() => setTab('promos')} className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all ${tab === 'promos' ? 'bg-slate-900 border-slate-700 text-slate-900' : 'bg-white/5 border-white/5 text-slate-500'}`}>{t('tab_promos')}</button>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            {selectedVenue.subscriptionTier === 'ticketing' || isAdmin ? (
              ...
              <div className="text-center p-8 bg-white border border-slate-200 rounded-2xl">
                ...
                <p className="text-slate-500 mb-6">Mejora tu plan a Ticketing (100€/mes) para organizar eventos y vender entradas.</p>
                <button onClick={() => handleSubscription('ticketing')} disabled={checkingOut} className="px-6 py-2 bg-slate-900 rounded-xl font-bold">Actualizar Plan</button>
              </div>
            )}
          </motion.div>
        )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'Entradas activas', value: validTickets, icon: 'local_activity', color: 'text-slate-800' },
        ...
        {tab === 'tickets' && (
          <div className="space-y-3">
            {selectedVenue?.subscriptionTier !== 'ticketing' && !isAdmin ? (
              <div className="text-center p-8 bg-white border border-slate-200 rounded-2xl">
                <h3 className="text-xl font-black mb-2">Requiere Tier Ticketing</h3>
                <p className="text-slate-500 mb-6">Mejora tu plan a Ticketing (100€/mes) para gestionar validación de entradas.</p>
                <button onClick={() => handleSubscription('ticketing')} disabled={checkingOut} className="px-6 py-2 bg-slate-900 rounded-xl font-bold">Actualizar Plan</button>
              </div>
            ) : tickets.length === 0 ? (
            ...
            {['promo', 'ticketing'].includes(selectedVenue?.subscriptionTier) || isAdmin ? (
              <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4">
                ...
                <textarea
                  ...
                  rows={3}
                  maxLength={200}
                  className="w-full bg-black/30 border border-white/10 rounded-2xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-600 focus:outline-none focus:border-slate-700/50 resize-none"
                />
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      className="bg-black/30 border border-white/10 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none"
                    >
              ...
              <div className="text-center p-8 bg-white border border-slate-200 rounded-2xl">
                ...
                <p className="text-slate-500 mb-6">Mejora tu plan a Promo o Ticketing (desde 60€/mes) para enviar notificaciones push promocionales a los usuarios cercanos.</p>
                <button onClick={() => handleSubscription('promo')} disabled={checkingOut} className="px-6 py-2 bg-slate-900 rounded-xl font-bold">Actualizar Plan</button>
              </div>
            )}
          </div>
        )}
