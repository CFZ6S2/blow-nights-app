...
export default function RRPPEventCard({ token }: { token: string }) {
  ...
  if (loading) {
    return (
      <div className="bg-white border border-white/5 rounded-3xl p-6 animate-pulse">
        <div className="h-6 bg-white/10 rounded-lg w-2/3 mb-4" />
  ...
  return (
    <>
      <div className={`bg-white border rounded-3xl overflow-hidden ${data.promoter.is_closed ? 'border-yellow-500/20' : 'border-slate-700/20'}`}>
        <div className="p-6">
            ...
            <div>
              ...
              {data.stats.totalSold === 0 ? (
                ...
                <span 
                  ...
                  className="inline-flex text-slate-500 bg-slate-100/50 px-3 py-1.5 rounded-xl text-[10px] font-bold border border-white/5 cursor-not-allowed items-center gap-1"
                >
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="bg-black/40 border border-white/5 p-4 rounded-2xl text-center">
              <div className="text-3xl font-black text-slate-800">{data.stats.totalSold}</div>
          ...
          {!data.promoter.is_closed ? (
            <div className="space-y-3">
              {data.promoter.qr_quota > 0 ? (
                <button
                  ...
                  className="w-full py-4 bg-slate-900 hover:bg-fuchsia-500 rounded-2xl font-black text-sm shadow-[0_0_30px_-10px_rgba(192,38,211,0.5)] transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                >
                  <span className="material-icons text-lg">qr_code</span>
                  {t('rrpp_event_card.issue_qr_ticket')}
                </button>
              ) : (
                <div className="bg-fuchsia-900/20 border border-slate-700/30 p-4 rounded-2xl text-center">
                  <p className="text-xs text-slate-500 mb-3">{t('rrpp_event_card.no_qr_balance')}</p>
            ...
            <div className="space-y-3">
              <div className="bg-black/40 border border-yellow-500/20 p-4 rounded-2xl text-center">
                ...
                <p className="text-xs text-slate-500">{t('rrpp_event_card.lists_closed')}</p>
              </div>
              <button
                ...
                className={`w-full py-4 rounded-2xl font-bold uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 ${
                  data.promoter.liquidated_by_rrpp
                    ...
                    : 'bg-slate-900 hover:bg-fuchsia-500 text-slate-900'
                }`}
              >
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white border border-white/10 w-full max-w-sm rounded-3xl p-6 relative overflow-hidden"
            >
              <button onClick={() => setShowModal(false)} className="absolute top-4 right-4 text-slate-500 hover:text-slate-900">
              ...
              {!generatedQR ? (
                <>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-widest">{t('rrpp_event_card.guest_alias')}</label>
                      <input
                        type="text"
                        value={clientName}
                        onChange={e => setClientName(e.target.value)}
                        placeholder={t('rrpp_event_card.guest_alias_placeholder')}
                        className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-700"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-widest">{t('rrpp_event_card.pass_type')}</label>
                      <select
                        value={tierId}
                        onChange={e => setTierId(e.target.value)}
                        className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-700 appearance-none"
                      >
                ...
                <div className="flex flex-col items-center py-4">
                  ...
                  <p className="text-slate-500 text-sm mb-6">{t('rrpp_event_card.pass_for')} {generatedQR.name}</p>
                  <div className="bg-white p-4 rounded-2xl mb-6 shadow-xl">
                    <QRCode value={generatedQR.token} size={200} />
                  </div>
                  <button
                    onClick={shareWhatsApp}
                    className="w-full py-4 bg-[#25D366] hover:bg-[#1ebe5d] text-slate-900 rounded-xl font-black flex items-center justify-center gap-2 mb-3"
                  >
