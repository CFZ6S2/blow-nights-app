...
export default function PremiumPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 p-6 pb-24 relative overflow-hidden">
      {/* Background elements */}
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-slate-900/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      <main className="max-w-6xl mx-auto w-full space-y-8 relative z-10">
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-white/10 backdrop-blur-md p-1 rounded-full inline-flex border border-white/20">
            <button 
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${period === 'monthly' ? 'bg-white text-black shadow-lg' : 'text-slate-900/70 hover:text-slate-900'}`}
            >
              {t('premium.monthly')}
            </button>
            <button 
              onClick={() => setPeriod('yearly')}
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-2 ${period === 'yearly' ? 'bg-gradient-to-r from-fuchsia-500 to-indigo-500 text-slate-900 shadow-lg' : 'text-slate-900/70 hover:text-slate-900'}`}
            >
        ...
        {isPromoLifetime && (
          <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 p-6 rounded-3xl text-center space-y-2 mb-8">
            ...
            <p className="text-sm text-slate-600">Fuiste uno de los 100 primeros. Disfruta de todos los beneficios Black gratis para siempre.</p>
          </div>
        )}
        ...
        {(isPlus || isBlack) && !isPromoLifetime && (
          <div className="bg-white/10 border border-white/20 p-6 rounded-3xl flex justify-between items-center mb-8 backdrop-blur-sm">
            <div>
              <p className="text-slate-500 text-xs uppercase tracking-widest font-bold">{t('premium.current_plan')}</p>
              <h3 className={`text-2xl font-black ${isBlack ? 'text-transparent bg-clip-text bg-gradient-to-r from-slate-300 to-white' : 'text-slate-800'}`}>
                {isBlack ? 'BLACK' : 'PLUS'}
              </h3>
            </div>
            <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          ...
          >
            <div className="space-y-2 mb-8">
              <h3 className="text-xl font-black uppercase tracking-widest text-slate-600">Free</h3>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black">0€</span>
              </div>
            </div>
            <ul className="space-y-4 text-xs font-bold text-slate-500 flex-grow">
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900/50 text-sm">check_circle</span> Mapa con usuarios cercanos</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900/50 text-sm">check_circle</span> Chat ilimitado con matches</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900/50 text-sm">check_circle</span> 3 pings al día</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900/50 text-sm">check_circle</span> Ver locales y eventos</li>
            </ul>
            {membershipTier === 'free' && !isPromoLifetime && (
              <div className="mt-8">
                <div className="bg-black/40 border border-indigo-500/30 p-4 rounded-2xl relative overflow-hidden group">
                  ...
                  <p className="text-[10px] text-slate-500 mb-3 leading-relaxed">Obtén todas las ventajas Black (excepto crear Chills) por una noche.</p>
                  ...
                  {timeLeft ? (
                    ...
                    <button
                      ...
                      className="w-full bg-indigo-600 hover:bg-indigo-500 text-slate-900 font-bold py-2 rounded-xl text-xs transition-all shadow-lg shadow-indigo-500/20"
                    >
                      Comprar por 5€
                    </button>
                  )}
          ...
          <motion.div 
            whileHover={{ y: -5 }}
            className={`bg-white/5 backdrop-blur-xl border p-8 rounded-[2rem] flex flex-col relative overflow-hidden transition-all ${isPlus ? 'border-slate-700 shadow-[0_0_30px_rgba(217,70,239,0.2)]' : 'border-white/10 hover:border-slate-700/50'}`}
          >
            <div className="space-y-2 mb-8">
              <h3 className="text-xl font-black uppercase tracking-widest text-slate-800">Plus</h3>
              <div className="flex items-baseline gap-1">
            ...
            <ul className="space-y-4 text-xs font-bold text-slate-600 flex-grow">
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">add_circle</span> <span className="text-slate-900">Todo lo Free</span></li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">visibility</span> Ver quién te ha visitado</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">visibility_off</span> Modo fantasma</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">bolt</span> 10 pings al día</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">rocket_launch</span> 1 boost gratis al mes</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">tune</span> Filtros avanzados</li>
              <li className="flex items-center gap-3"><span className="material-icons text-slate-900 text-sm">favorite</span> Ver quién te dio like</li>
            </ul>
            <div className="mt-8">
              {!isPlus && !isBlack && !isPromoLifetime && (
                <button
              ...
              {isPlus && (
                <div className="text-center py-4 bg-fuchsia-500/10 rounded-2xl border border-slate-700/30">
                  <span className="text-[10px] font-bold text-slate-800 uppercase">Tu plan actual</span>
                </div>
              )}
            </div>
          </motion.div>
          ...
          <motion.div 
            whileHover={{ y: -5 }}
            className={`bg-gradient-to-br from-slate-900 to-black backdrop-blur-xl border p-8 rounded-[2rem] flex flex-col relative overflow-hidden transition-all shadow-2xl ${isBlack || isPromoLifetime ? 'border-slate-400 shadow-[0_0_40px_rgba(255,255,255,0.15)]' : 'border-slate-300 hover:border-slate-500'}`}
          >
            {/* VIP Badge effect */}
            ...
            <div className="space-y-2 mb-8">
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black text-slate-900">{period === 'monthly' ? '20€' : '139,99€'}</span>
                <span className="text-slate-500 text-xs font-black">/{period === 'monthly' ? 'mes' : 'año'}</span>
              </div>
            </div>
            <ul className="space-y-4 text-xs font-bold text-slate-600 flex-grow">
              <li className="flex items-center gap-3"><span className="material-icons text-slate-100 text-sm">add_circle</span> <span className="text-slate-900">Todo lo Plus</span></li>
              ...
              <li className="flex items-center gap-3 font-black text-slate-900 bg-white/10 p-2 rounded-xl border border-white/20"><span className="material-icons text-sm">nightlife</span> Crear Chills y Afters</li>
            ...
            <div className="mt-8">
              ...
              {(isBlack || isPromoLifetime) && (
                <div className="text-center py-4 bg-white/10 rounded-2xl border border-white/30">
                  <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Tu plan actual</span>
                </div>
              )}
            </div>
          </motion.div>
