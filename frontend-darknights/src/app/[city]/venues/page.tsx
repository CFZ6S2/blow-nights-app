...
function VenueCounter({ venueId }: { venueId: string }) {
  ...
  return (
    <span className="bg-fuchsia-500/20 text-slate-800 text-[10px] font-black px-2 py-0.5 rounded-full">
...
export default function VenuesPage() {
  ...
  if (venuesLoading) return <div className="min-h-screen bg-slate-50 flex justify-center items-center"><div className="w-8 h-8 border-2 border-slate-700 rounded-full animate-spin border-t-transparent" /></div>;
      ...
      <main className="relative z-10 p-6 md:p-12 max-w-2xl mx-auto space-y-6">
        ...
        <div className="relative w-full sm:w-64">
          <select
            ...
            className="w-full bg-white border border-slate-300 text-slate-700 text-sm rounded-xl px-3 py-2.5 appearance-none focus:outline-none focus:border-purple-500 transition-all cursor-pointer font-medium"
          >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center flex-shrink-0">
                      <span className="material-icons text-slate-800">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {venue.ownerId === 'community' ? (
                          <span className="bg-slate-700/50 text-slate-600 text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider">{t('venuesPage.addedByCommunity')}</span>
                        ) : (
                  ...
                  {activeCheckin?.venueId === venue.id ? (
                    ...
                    <button
                      ...
                      disabled={checkingIn}
                      className="ml-3 px-4 py-2.5 rounded-2xl bg-slate-900 text-slate-900 text-[10px] font-black uppercase tracking-widest hover:bg-fuchsia-500 transition-all active:scale-95 disabled:opacity-50 flex-shrink-0"
                    >
                  ...
                  <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                    <div className="flex gap-2 flex-wrap">
                      {venue.ticketPricing && Object.entries(venue.ticketPricing).map(([type, pricing]: [string, any]) => (
                        <span key={type} className="text-[9px] bg-white/5 text-slate-500 px-2 py-1 rounded-lg">
                          {pricing.name || type}: {(pricing.amount / 100).toFixed(2)}€
                        </span>
                      ))}
                      {venue.ticket_tiers && venue.ticket_tiers.map((tier: any) => (
                        <span key={tier.id} className="text-[9px] bg-white/5 text-slate-500 px-2 py-1 rounded-lg">
