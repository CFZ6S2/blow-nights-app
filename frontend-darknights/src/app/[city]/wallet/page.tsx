  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-32">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-[20%] -right-[10%] w-[40%] h-[40%] bg-slate-900/10 blur-[120px] rounded-full" />
      </div>
          ...
          <div className="text-center py-16 space-y-4">
            ...
            {tab === 'valid' && (
              <button
                ...
                className="px-6 py-3 rounded-2xl bg-slate-900 text-slate-900 text-xs font-black uppercase tracking-widest"
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                      ticket.status === 'valid' ? 'bg-green-500/20' : 'bg-slate-100'
                    }`}>
        ...
        {selectedTicket && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-sm bg-white border border-white/10 rounded-[3rem] p-8 space-y-6 shadow-[0_40px_100px_rgba(0,0,0,0.8)]"
            >
              ...
              <div id="ticket-content" className="print:text-black">
                ...
                <div className="space-y-2 print:hidden">
                  <button onClick={() => window.print()} className="w-full py-3 bg-slate-900 hover:bg-fuchsia-500 text-slate-900 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-colors">
                ...
                <div className="mt-4 p-4 bg-slate-50 print:bg-white print:border print:border-slate-300 rounded-xl text-[8px] text-slate-500 print:text-slate-800 leading-tight space-y-2 max-h-32 overflow-y-auto print:max-h-none text-left">
                  <p className="font-bold uppercase text-slate-500 print:text-black">{t('wallet.conditions_title')}</p>
              ...
              <button
                onClick={() => setSelectedTicket(null)}
                className="w-full mt-6 py-4 rounded-2xl bg-white/5 text-slate-500 font-black uppercase tracking-[0.2em] text-xs hover:bg-white/10 transition-all print:hidden"
              >
                {t('wallet.close')}
              </button>
            </motion.div>
