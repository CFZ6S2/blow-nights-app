...
export default function VisitsPage() {
  ...
  if (authLoading || loading) {
    return (
      <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 p-8">
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 pb-32 overflow-x-hidden">
      <AnimatePresence>
        {selectedProfileId && (
      ...
      <header className="sticky top-0 z-40 bg-slate-50/60 backdrop-blur-3xl border-b border-white/5 px-8 py-8">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-fuchsia-500/10 border border-slate-700/20 rounded-xl">
            <span className="text-slate-800 font-black text-xs">{visits.length}</span>
            <span className="material-icons text-slate-900 text-sm">visibility</span>
          </div>
        </div>
      </header>
      ...
      <div className="px-6 pt-8 space-y-4 max-w-2xl mx-auto w-full">
        {visits.length === 0 ? (
          ...
          >
            ...
            <p className="text-xl font-black text-slate-500">{t('visits.empty_title')}</p>
          ...
          <div className="grid gap-3">
            {visits.map((visit, idx) => (
              ...
              >
                {!isPremium && idx > 2 && (
                  <div className="absolute inset-0 bg-slate-50/40 backdrop-blur-md z-10 flex items-center justify-center">
                ...
                <div className={`relative w-20 h-20 rounded-[1.8rem] overflow-hidden border-2 transition-all ${isPremium ? 'border-slate-700/30' : 'border-slate-200'}`}>
                  <img 
        ...
        {!isPremium && visits.length > 0 && (
          ...
          >
            ...
            <p className="text-slate-500 text-sm leading-relaxed mb-8 px-4">
              {t('visits.there_are')} <span className="text-slate-900 font-bold">{visits.length} {t('visits.people', { count: visits.length })}</span> {t('visits.fans_promo_text')}
            </p>
