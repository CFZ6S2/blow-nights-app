...
export default function ChillDetailClient({ chillId }: { chillId: string }) {
  ...
  if (!chill) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center">
        ...
        <button onClick={() => router.push(cityPath('/chills'))} className="px-6 py-3 bg-slate-900 rounded-xl font-bold text-sm">
  ...
  return (
    <div className="min-h-screen bg-black text-slate-900 pb-28">
      <div className="max-w-md mx-auto px-4 pt-6">
        <button onClick={() => router.back()} className="flex items-center gap-1 text-slate-500 hover:text-slate-900 mb-4 transition-colors">
          <span className="material-icons text-sm">arrow_back</span>
          <span className="text-xs font-bold">{t('chill.back')}</span>
        </button>

        <div className="bg-white/80 border border-white/5 rounded-3xl p-6 mb-4">
          <div className="flex items-start gap-4 mb-4">
            <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-slate-700/40 flex-shrink-0 cursor-pointer"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500 font-bold">
                {t('chill.organized_by')} <span className="text-slate-800">{chill.host_nick}</span>
              </p>
            </div>
            <div className="flex flex-col items-end gap-1">
              <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                chill.status === 'ended' ? 'bg-slate-700 text-slate-500'
          ...
          {chill.description && (
            <p className="text-sm text-slate-600 mb-4">{chill.description}</p>
          )}

          {chill.tags && chill.tags.length > 0 && (
            <div className="flex gap-2 flex-wrap mb-4">
              {chill.tags.map((tag) => (
                <span key={tag} className="text-[10px] bg-white/5 text-slate-500 px-2 py-0.5 rounded-full font-bold">
                  {tag}
                </span>
              ))}
            </div>
          ...
          {canSeeAddress ? (
            <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-900 font-bold">{exactAddress || t('chill.loading')}</p>
              </div>
            </div>
          ) : (
            <div className="bg-slate-100/50 border border-white/5 rounded-2xl p-4 flex items-start gap-3">
            ...
            {isAccepted ? (
              ...
              <>
                <button
                  ...
                  className="w-full py-5 bg-slate-900 hover:bg-fuchsia-500 disabled:bg-slate-100 disabled:text-slate-600 rounded-2xl font-black text-lg shadow-[0_0_40px_-10px_rgba(192,38,211,0.5)] transition-all active:scale-95 flex items-center justify-center gap-2"
                >
            ...
            {tab === 'requests' && isHost && (
              <div className="space-y-3">
                {requests.length === 0 ? (
                  ...
                  requests.map((req) => (
                    <div key={req.id} className="bg-white/80 border border-white/5 rounded-2xl p-4">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {req.user_bio && <p className="text-xs text-slate-500 line-clamp-1 mt-0.5">{req.user_bio}</p>}
                        </div>
                      </div>
              ...
              <div className="flex flex-col" style={{ height: 'calc(100dvh - 420px)', minHeight: '300px' }}>
                      ...
                      return (
                        <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : ''}`}>
                          ...
                          <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'}`}>
                            {!isMe && <p className="text-[10px] text-slate-800 font-bold mb-0.5 px-1">{msg.nick}</p>}
                            <div className={`px-3 py-2 rounded-2xl text-sm ${
                              isMe
                                ? 'bg-slate-900 text-slate-900 rounded-br-sm'
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-white border border-white/10 rounded-xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-slate-700"
                  />
                  <button
                    onClick={sendMessage}
                    disabled={!msgText.trim()}
                    className="w-12 h-12 rounded-xl bg-slate-900 hover:bg-fuchsia-500 disabled:bg-slate-100 flex items-center justify-center transition-all"
                  >
                    <span className="material-icons text-slate-900">send</span>
                  </button>
                </div>
              </div>
            )}
