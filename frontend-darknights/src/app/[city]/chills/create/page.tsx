...
export default function CreateChillPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-slate-900 pb-20 pt-8 px-4">
      <div className="max-w-md mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className={`p-4 rounded-2xl border-2 transition-all flex items-start gap-4 text-left ${
                type === 'social' 
                  ? 'border-slate-700 bg-fuchsia-500/10' 
                  : 'border-gray-800 bg-gray-900 hover:border-gray-700'
              }`}
            >
              <div className={`p-3 rounded-full ${type === 'social' ? 'bg-fuchsia-500 text-slate-900 shadow-[0_0_15px_rgba(217,70,239,0.5)]' : 'bg-gray-800 text-gray-400'}`}>
                <Users size={24} />
              </div>
              <div>
                <h3 className={`font-bold text-lg ${type === 'social' ? 'text-slate-800' : 'text-slate-900'}`}>
            ...
            >
              <div className={`p-3 rounded-full ${type === 'commercial' ? 'bg-amber-500 text-slate-900 shadow-[0_0_15px_rgba(245,158,11,0.5)]' : 'bg-gray-800 text-gray-400'}`}>
                <Sparkles size={24} />
              </div>
              <div>
                <h3 className={`font-bold text-lg ${type === 'commercial' ? 'text-amber-400' : 'text-slate-900'}`}>
          ...
          <div className="space-y-4 pt-2">
            <div>
              ...
              <input
                ...
                className="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-fuchsia-500 outline-none"
              />
            </div>
              ...
              {type === 'commercial' && (
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-900 border border-amber-500/50 rounded-xl px-4 py-3 text-slate-900 focus:ring-2 focus:ring-amber-500 outline-none"
                  />
                </div>
              )}
            </div>
            ...
            <div className="pt-2 border-t border-gray-800">
              ...
              {!userLocation ? (
                <button
                  ...
                  className="w-full py-3 bg-gray-800 hover:bg-gray-700 text-slate-900 rounded-xl flex items-center justify-center gap-2 transition"
                >
                  <MapPin size={18} />
          ...
          <div className="bg-gray-900 border border-gray-800 p-4 rounded-xl mt-4">
            <label className="flex items-start gap-3 cursor-pointer">
              <input 
                ...
                className="mt-1 w-5 h-5 rounded border-gray-700 bg-gray-800 text-slate-900 focus:ring-fuchsia-500/50"
              />
              <span className="text-xs text-gray-400 leading-relaxed">
                <strong className="text-slate-900">{t('chills.create.terms_strong')}</strong> {t('chills.create.terms_text')}
              </span>
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-4 rounded-xl font-bold text-slate-900 shadow-xl transition-all mt-4 ${
              loading ? 'opacity-70 cursor-not-allowed' : 'hover:scale-[1.02]'
            } ${
              type === 'commercial' 
                ? 'bg-gradient-to-r from-amber-500 to-orange-500 shadow-[0_0_20px_rgba(245,158,11,0.3)] text-gray-950' 
                : 'bg-gradient-to-r from-slate-800 to-purple-600 shadow-[0_0_20px_rgba(217,70,239,0.3)]'
            }`}
          >
