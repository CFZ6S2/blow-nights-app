...
function ChillCard({ chill, userLat, userLng, onClick }: {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -12 }}
      onClick={onClick}
      className="bg-white/80 border border-white/5 rounded-3xl p-5 cursor-pointer hover:border-slate-700/30 transition-all active:scale-[0.98]"
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-slate-700/40 flex-shrink-0">
            {chill.host_foto ? (
          ...
          <div>
            <h3 className="font-black text-slate-900 text-sm leading-tight">{chill.title}</h3>
      ...
      {chill.description && (
        <p className="text-xs text-slate-500 mb-3 line-clamp-2">{chill.description}</p>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1 ml-auto">
          <span className="material-icons text-slate-900/60 text-xs">group</span>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="material-icons text-slate-800 text-xl">local_fire_department</span>
            </div>
            <CitySelector />
          </div>
          <button
            onClick={handleCreateClick}
            className="w-12 h-12 rounded-2xl bg-slate-900 hover:bg-fuchsia-500 flex items-center justify-center shadow-lg shadow-fuchsia-500/20 transition-all active:scale-90"
          >
            <span className="material-icons text-slate-900">add</span>
          </button>
        </div>
          ...
          <div className="space-y-3">
            {sorted.length === 0 ? (
              <div className="text-center py-16">
                ...
                <button
                  onClick={handleCreateClick}
                  className="mt-6 px-6 py-3 bg-slate-900 hover:bg-fuchsia-500 rounded-xl font-black text-sm transition-all"
                >
