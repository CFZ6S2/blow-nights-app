...
function ChillManageContent() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#0a0b12] flex items-center justify-center text-slate-900 text-xs">Cargando panel...</div>;
  }

  if (!chill) {
    return <div className="min-h-screen bg-[#0a0b12] flex items-center justify-center text-slate-900 text-xs">Evento no encontrado.</div>;
  }
  ...
  return (
    <div className="max-w-xl mx-auto p-4 space-y-4 text-slate-900 bg-[#0a0b12] min-h-screen">
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="bg-[#121422] border border-[#2b2f4c] p-2.5 rounded-xl">
          ...
          <span className="text-lg font-black text-slate-800">{chill.stats?.address_reveals || 0}</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {requests.filter(r => r.status?.toLowerCase() === 'pending').map(req => (
          <div key={req.id} className="bg-[#121422] border border-[#23273f] p-3 rounded-xl flex items-center justify-between gap-2">
            <div>
              <p className="text-sm font-bold text-slate-900">{req.userName || req.user_nick}</p>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-500 text-slate-900 text-[11px] font-bold px-3 py-2 rounded-lg transition"
              >
                Pase QR (+1€)
              </button>
              <button
...
export default function ChillManagePage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0a0b12] flex items-center justify-center text-slate-900 text-xs">Cargando panel...</div>}>
      <ChillManageContent />
    </Suspense>
  );
}
