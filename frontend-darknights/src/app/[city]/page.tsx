      ...
      <header className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-medium">{t('city.hello')}, {profile.nick}! {isAdmin && <span className="text-purple-400 font-black ml-1">[ADMIN]</span>}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <CitySelector />
          {isSuperAdmin && (
            <Link href="/super-admin" prefetch={false} className="bg-slate-900/20 text-slate-800 border border-fuchsia-600/50 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase hover:bg-slate-900/30 transition-all">
              {t('city.super_admin')}
            </Link>
          )}
          {isAdmin && (
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-2 gap-4">
          <motion.div variants={itemVariants} whileTap={{ scale: 0.95 }} className="h-full">
            ...
            >
              <div className="text-3xl mb-3 group-hover:scale-110 transition-transform relative z-10">
                <span className="material-icons text-slate-800 text-3xl drop-shadow-[0_0_8px_rgba(192,38,211,0.5)]">nightlife</span>
              </div>
              <h3 className="font-display font-bold relative z-10">{t('city.where_going')}</h3>
              <p className="text-[10px] text-slate-500 mt-1 relative z-10">{t('city.choose_spot')}</p>
              <div className="absolute inset-0 bg-gradient-to-br from-slate-800/10 to-violet-600/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            </Link>
          </motion.div>
          ...
          <motion.div variants={itemVariants} whileTap={{ scale: 0.95 }} className="h-full">
            ...
            >
              ...
              <p className="text-[10px] text-slate-500 mt-1 relative z-10">{t('city.tickets_passes')}</p>
          ...
          <motion.div variants={itemVariants} whileTap={{ scale: 0.95 }} className="h-full">
            ...
            >
              ...
              <p className="text-[10px] text-slate-500 mt-1 relative z-10">{t('city.pending_messages')}</p>
            </Link>
          </motion.div>
          ...
          <motion.div variants={itemVariants} whileTap={{ scale: 0.95 }} className="h-full">
            ...
            >
              ...
              <p className="text-[10px] text-slate-500 mt-1 relative z-10">{t('city.who_saw_you')}</p>
            </Link>
          </motion.div>
            ...
            >
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">{t('city.take_home')}</p>
                </div>
              </div>
              <span className="material-icons text-slate-500 group-hover:text-slate-900 transition-colors relative z-10">download</span>
              <div className="absolute inset-0 bg-gradient-to-r from-slate-800/10 to-indigo-600/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          </motion.div>
