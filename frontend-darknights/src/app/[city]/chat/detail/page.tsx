...
function ChatDetailContent() {
  ...
  if (loading || !otherUser) return (
    <div className="flex items-center justify-center min-h-screen bg-slate-50">
      <motion.div 
        ...
        className="h-10 w-10 border-t-2 border-slate-700 rounded-full"
      />
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 overflow-hidden">
      {/* Header Premium */}
      <header className="px-6 py-6 bg-white/40 backdrop-blur-3xl border-b border-white/5 flex items-center gap-4 z-40">
        <motion.button 
          whileTap={{ scale: 0.9 }}
            ...
            <AnimatePresence mode="wait">
              {isOtherTyping ? (
                <motion.p 
                  ...
                  exit={{ opacity: 0, x: 5 }}
                  className="text-[9px] font-black uppercase tracking-widest text-slate-800 animate-pulse"
                >
                  {t('chat.typing')}
                </motion.p>
                ...
                <motion.p 
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: 5 }}
                  className="text-[9px] font-black uppercase tracking-widest text-slate-900 flex items-center gap-1"
                >
          ...
          {showOptions && (
            <>
              ...
              <motion.div 
                ...
                className="absolute top-24 right-6 w-56 bg-white/90 backdrop-blur-2xl border border-white/10 rounded-3xl p-2 z-40 shadow-2xl"
              >
                <button onClick={() => { triggerHaptic(10); router.push(cityPath(`/profile/view?id=${otherUser.id || searchParams.get('otherId')}`)); }} className="w-full p-4 flex items-center gap-3 text-sm font-bold text-slate-700 hover:bg-white/5 rounded-2xl transition-all">
                  <span className="material-icons text-slate-500">person</span> {t('chat.view_profile')}
                </button>
                <button 
                  onClick={async () => { 
                    triggerHaptic(10); 
      ...
      >
            ...
            return (
              ...
              >
                <div className={`max-w-[80%] p-4 rounded-3xl relative group transition-all duration-300 ${
                  isMe 
                  ? 'bg-gradient-to-br from-slate-800 to-purple-600 text-slate-900 rounded-tr-none shadow-[0_10px_20px_rgba(217,70,239,0.15)]' 
                  : 'bg-slate-100/80 backdrop-blur-xl border border-white/5 rounded-tl-none text-slate-100 shadow-xl'
                }`}>
                  {/* Bubble Tail */}
                  <div className={`absolute top-0 w-4 h-4 ${
                    isMe 
                    ? 'right-[-8px] bg-purple-600 [clip-path:polygon(0_0,0_100%,100%_0)]' 
                    : 'left-[-8px] bg-slate-100/80 [clip-path:polygon(100%_0,100%_100%,0_0)]'
                  }`} />
                  ...
                  <div className="flex items-center justify-end gap-1.5 mt-2 opacity-60">
                    ...
                    {isMe && (
                      <span className={`material-icons text-[14px] ${msg.read ? 'text-blue-400' : 'text-slate-900/40'}`}>
        ...
        {isOtherTyping && (
          <div className="flex justify-start">
            <div className="bg-slate-100/40 px-6 py-4 rounded-3xl rounded-tl-none flex gap-1.5">
              {[0, 1, 2].map(i => (
      ...
      {/* Input v33 MediaFlow */}
      <footer className="p-6 pb-12 bg-white/80 backdrop-blur-3xl border-t border-white/5 relative">
        {uploading && (
          <motion.div 
            initial={{ width: 0 }}
        ...
        <form onSubmit={handleSend} className="max-w-4xl mx-auto flex items-end gap-3">
          ...
          <motion.button
            ...
            className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center border border-white/5 hover:bg-white/10 transition-all text-slate-500"
          >
            <span className="material-icons">add_a_photo</span>
          </motion.button>

          <div className="flex-1 bg-white/5 rounded-[2rem] border border-white/5 p-2 flex items-end gap-2 focus-within:border-slate-700/30 transition-all relative overflow-hidden">
            {isRecording ? (
          ...
          <AnimatePresence mode="wait">
            <motion.button 
              ...
              className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                ...
                : (newMessage.trim() ? 'bg-slate-900 text-slate-900 shadow-lg shadow-fuchsia-500/20' : 'bg-white/5 text-slate-500')
              }`}
            >
