...
function PublicProfileContent() {
  ...
  const handleChat = async () => {
    ...
    // Si no es VIP, comprobamos si tienen un match mutuo en la colección 'matches'
  ...
  if (!profile) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
      <h2 className="text-2xl font-black mb-4 text-slate-900">{t('profileView.profileNotFound')}</h2>
      <button onClick={() => router.back()} className="text-slate-900 font-bold uppercase tracking-widest">{t('profileView.back')}</button>
    </div>
  );
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="min-h-screen bg-slate-50 text-slate-900 pb-40"
    >
      <AnimatePresence>
      ...
      {pingVenue && (
        <div className="fixed top-0 left-0 right-0 z-50 p-4 pt-8 bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
          <motion.div 
            ...
            className="bg-white/90 backdrop-blur-md border border-orange-500/50 rounded-2xl p-4 shadow-2xl pointer-events-auto"
          >
            <div className="flex items-center gap-3 mb-3">
              <span className="text-3xl">🔥</span>
              <div>
                <p className="text-slate-900 text-xs font-bold">{profile.nick} {t('profileView.invitedToDate')}</p>
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="flex-1 py-2.5 rounded-xl bg-white/10 text-slate-900 font-bold text-[10px] uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-white/20 transition-all"
              >
                <span className="material-icons text-sm">map</span> {t('profileView.howToGetThere')}
              </button>
              <button 
                onClick={() => handleLike()}
                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-slate-900 font-black text-[10px] uppercase tracking-wider flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(245,158,11,0.5)] hover:scale-105 transition-all"
              >
      ...
      <div className="relative z-10 pt-[55vh]">
        <div className="bg-slate-50/80 backdrop-blur-xl border-t border-white/5 rounded-t-[3rem] p-8 shadow-[0_-20px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <div className="px-4 py-2 rounded-xl bg-white/5 border border-white/5 text-[10px] font-bold text-slate-600 uppercase tracking-wider">{profile.rol === 'party_only' ? t('profileView.onlyParty') : profile.rol}</div>
            <div className="px-4 py-2 rounded-xl bg-fuchsia-500/10 border border-slate-700/10 text-[10px] font-bold text-slate-800 uppercase tracking-wider">{profile.intencion}</div>
          ...
          <div className="space-y-8">
              ...
              <div className="flex flex-wrap gap-2">
                {intereses.map((interes: string, idx: number) => (
                  <motion.span 
                    ...
                    className="px-4 py-2 rounded-xl bg-white/5 border border-white/5 text-[10px] font-bold text-slate-500 hover:bg-white/10 transition-colors cursor-default"
                  >
                    #{interes}
                  </motion.span>
                ))}
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-600 leading-relaxed text-lg font-medium bg-white/5 p-6 rounded-3xl border border-white/5">
            ...
            {(profile.privatePhotos && profile.privatePhotos.length > 0) && (
              <div className="space-y-4">
                ...
                {profile.permittedUsers?.includes(currentUser?.uid) ? (
                  ...
                  <button 
                    onClick={handleChat}
                    className="w-full p-8 rounded-[2rem] bg-white/5 border border-dashed border-yellow-500/20 flex flex-col items-center justify-center gap-3 group hover:bg-yellow-500/5 transition-all"
                  >
                    ...
                    <div className="text-center">
                      <p className="text-sm font-bold text-slate-900">{t('profileView.privateContent')}</p>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-30">
        <div className="bg-white/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-3 flex gap-3 shadow-[0_25px_60px_rgba(0,0,0,0.8)]">
          <motion.button 
            whileTap={{ scale: 0.9 }} 
          ...
          <motion.button 
            ...
            className={`flex-1 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-xl ${hasLiked ? 'bg-slate-100 text-slate-500' : 'bg-gradient-to-r from-slate-800 via-purple-600 to-indigo-600 text-slate-900 shadow-fuchsia-500/20'}`}
          >
          ...
          <motion.button 
            whileTap={{ scale: 0.9 }} 
            onClick={handleChat} 
            className="relative w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-slate-900 overflow-hidden group"
          >
            <motion.div 
