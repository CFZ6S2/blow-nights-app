    ...
    <div className="mt-4 pt-4 border-t border-white/5 space-y-4">
      ...
      <div className="flex gap-2">
        <input 
          ...
          className="flex-1 bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-indigo-500"
        />
        <button onClick={handleAddPromoter} className="bg-indigo-600 text-slate-900 px-4 py-2 rounded-xl text-sm font-bold">
...
export default function OrganizerDashboard() {
  ...
  if (authLoading || loading) return <div className="min-h-screen bg-slate-50 flex justify-center items-center"><div className="w-8 h-8 border-2 border-indigo-500 rounded-full animate-spin border-t-transparent" /></div>;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-12">
      <main className="relative z-10 p-6 md:p-12 max-w-4xl mx-auto space-y-8">
        <header className="pt-8">
          ...
          <p className="text-slate-500 mt-2">{t('organizer_page.manage_events_desc')}</p>
        </header>

        {events.length > 0 && !isCreating && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gradient-to-br from-fuchsia-900/40 to-slate-900/40 border border-slate-700/30 p-6 rounded-3xl relative">
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-800 mb-1">Créditos QR</div>
              <div className="text-3xl font-black text-slate-900">{(profile as any)?.qr_quota || 0}</div>
              <button 
                onClick={() => setBuyQRModal(true)}
                className="absolute top-1/2 -translate-y-1/2 right-6 bg-slate-900 hover:bg-fuchsia-500 text-slate-900 px-4 py-2 rounded-xl text-xs font-bold"
              >
                Recargar
              </button>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-900/40 to-slate-900/40 border border-purple-500/30 p-6 rounded-3xl">
              ...
              <div className="text-3xl font-black text-slate-900">{totalCheckedIn}</div>
            </div>
          </div>
        )}
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold text-slate-900">{t('organizer_page.my_events')}</h2>
          <button
            onClick={() => setIsCreating(!isCreating)}
            className="px-6 py-3 bg-indigo-600 text-slate-900 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-indigo-500 transition-colors"
          >
        ...
        {isCreating && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-indigo-500/30 rounded-3xl p-8 space-y-6">
            <h3 className="text-xl font-bold">{t('organizer_page.create_event')}</h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Título del Evento</label>
                <input 
                  type="text" 
                  value={title} 
                  onChange={e => setTitle(e.target.value)}
                  className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-indigo-500"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Ciudad (ID)</label>
                  <input 
                    type="text" 
                    value={cityId} 
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Cartel / Flyer (Opcional)</label>
                <input 
                  type="file" 
                  accept="image/*"
                  onChange={handleFlyerChange}
                  className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-indigo-600 file:text-slate-900 hover:file:bg-indigo-700"
                />
                {flyerPreview && (
              ...
              <button
                ...
                disabled={saving}
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-slate-900 rounded-2xl font-black uppercase tracking-widest transition-colors mt-6 shadow-[0_0_20px_rgba(79,70,229,0.3)] disabled:opacity-50"
              >
        ...
        <div className="space-y-4">
          {events.length === 0 && !isCreating && (
            <div className="text-center py-20 bg-white/5 rounded-3xl border border-white/5">
              ...
              <p className="text-lg font-bold text-slate-600">Aún no has creado ningún evento</p>
          ...
          {events.map(evt => (
            <div key={evt.id} className="bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col md:flex-row gap-6">
              {evt.banner_url ? (
                ...
                <div className="w-full md:w-48 h-48 bg-white rounded-2xl flex items-center justify-center">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="pt-4 border-t border-white/5">
                  <div className="flex items-center justify-between bg-black/40 p-4 rounded-xl border border-indigo-500/20">
                    <div>
                      ...
                      <div className="text-xs text-slate-500 font-mono mt-1">/door?eventId={evt.id}&token={evt.scanner_token.substring(0,8)}...</div>
                    </div>
                    <button 
                    ...
                    >
                      <span className="material-icons text-slate-900">content_copy</span>
                    </button>
                  </div>
                </div>
                ...
                <div className="pt-4 mt-4 border-t border-white/5 flex gap-2">
                  <button 
                    ...
                    className="flex-1 bg-slate-900 hover:bg-fuchsia-500 text-slate-900 py-2 rounded-xl text-sm font-bold flex items-center justify-center gap-2"
                  >
        ...
        {buyQRModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-white border border-white/10 rounded-3xl p-6 max-w-sm w-full space-y-6">
              <div className="flex justify-between items-center">
              ...
              <p className="text-sm text-slate-500">Añade saldo a tu cuenta para emitir entradas manualmente. Coste: 1€ por QR.</p>
        ...
        {issueModalOpen && !issuedQR && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-white border border-white/10 rounded-3xl p-6 max-w-sm w-full space-y-6">
              ...
              <p className="text-sm text-slate-500">Esta acción descontará 1 crédito de tu saldo ({(profile as any)?.qr_quota || 0} disponibles).</p>
              
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Nombre del Cliente</label>
                  <input 
                    type="text" 
                    value={issueClientName} 
                    onChange={e => setIssueClientName(e.target.value)}
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-slate-900 focus:outline-none"
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-slate-900 focus:outline-none appearance-none"
                  >
              ...
              <button
                ...
                className="w-full py-4 bg-slate-900 hover:bg-fuchsia-500 text-slate-900 rounded-2xl font-black uppercase tracking-widest disabled:opacity-50"
              >
