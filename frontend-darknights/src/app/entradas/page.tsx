...
export default function DarkNightsPublicEntradasPage() {
    ...
    const fetchPublicVenues = async () => {
      try {
        ...
        const fetched = snapshot.docs.map(doc => {
          ...
          return {
            id: doc.id,
            nombre: data.nombre || 'Dark Venue',
            ciudad: data.ciudad || 'madrid',
            fotoUrl: data.fotoUrl || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=800&auto=format&fit=crop',
        ...
        if (fetched.length === 0) {
          setVenues([
            { id: '1', nombre: 'Dark Chamber Club', ciudad: 'madrid', fotoUrl: 'https://images.unsplash.com/photo-1566737236500-c8ac43014a67?w=800&auto=format&fit=crop', direccion: 'Chueca 12', rating: 4.9, slug: 'dark-chamber-club', hasTickets: true },
            { id: '2', nombre: 'Underground Fetish Vault', ciudad: 'madrid', fotoUrl: 'https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=800&auto=format&fit=crop', direccion: 'Malasaña 8', rating: 4.8, slug: 'underground-vault', hasTickets: true },
            { id: '3', nombre: 'Black Velvet Room', ciudad: 'barcelona', fotoUrl: 'https://images.unsplash.com/photo-1574391884720-bbc3740c59d1?w=800&auto=format&fit=crop', direccion: 'Eixample 24', rating: 4.7, slug: 'black-velvet-room', hasTickets: true }
          ]);
        } else {
          setVenues(fetched);
        }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white selection:bg-red-500/30">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-2xl border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-red-600 to-amber-600 rounded-2xl flex items-center justify-center shadow-lg shadow-red-500/20">
              <Ticket className="w-5 h-5 text-white" />
            </div>
            <span className="text-xl font-black tracking-tight">DarkNights <span className="text-red-500">Entradas</span></span>
          </Link>
      ...
      <main className="pt-32 pb-24 px-6 max-w-7xl mx-auto space-y-12">
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-tight">
            Consigue tu <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-orange-500 to-amber-500">entrada secreta</span>
          </h1>
          <p className="text-slate-400 text-base md:text-lg">
            Pases VIP y tickets directos con QR dinámico tokenizado.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-900/60 p-4 rounded-3xl border border-white/10 backdrop-blur-xl">
          <div className="relative w-full md:w-96">
            ...
            <input
              type="text"
              placeholder="Buscar local nocturno..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-950 border border-white/10 rounded-2xl pl-12 pr-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-red-500 transition-colors"
            />
          </div>
              ...
              >
                ...
                <div className="p-6 space-y-4 bg-slate-900">
                  <div className="flex items-center justify-between text-xs text-slate-400 font-medium border-b border-white/5 pb-4">
                    ...
                    <span className="text-red-400 font-bold">Venta Activa</span>
                  </div>

                  <Link
                    href={`/${venue.slug}/entradas`}
                    className="w-full py-4 bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-red-500/20 transition-all flex items-center justify-center gap-2 group-hover:gap-3"
                  >
      ...
      <footer className="py-8 border-t border-white/5 text-center text-slate-600 text-xs font-medium">
        © {new Date().getFullYear()} DarkNights Ticketing Direct. Compra segura verificada.
      </footer>
    </div>
  );
}
