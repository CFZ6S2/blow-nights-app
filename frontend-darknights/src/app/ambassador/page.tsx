    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-12 pb-32">
      <header className="mb-8">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">{profile.nick || (profile as any).email || ''} · DarkNights Partner</p>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">{t('ambassador.totalAccumulated')}</p>
          <p className="text-3xl font-black font-mono">{totalEarned.toFixed(2)} <span className="text-lg text-slate-500">€</span></p>
        </div>
        <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">{t('ambassador.capturedCities')}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <section className="bg-white/5 border border-white/10 p-6 rounded-2xl">
          <h2 className="text-sm font-black uppercase tracking-widest text-slate-600 mb-4">{t('ambassador.myCities')}</h2>
            ...
            {affiliatedCities.map(c => (
              <div key={c.id} className="bg-black/30 border border-white/5 p-4 rounded-xl flex justify-between items-center">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest">{c.country || ''} · {c.managerId ? t('ambassador.withCM') : t('ambassador.withoutCM')}</p>
                </div>
        ...
        <section className="bg-white/5 border border-white/10 p-6 rounded-2xl h-[60vh] flex flex-col">
          <h2 className="text-sm font-black uppercase tracking-widest text-slate-600 mb-4">{t('ambassador.commissionHistory')}</h2>
