...
export default function DarkNightsVenueTicketsPage() {
    ...
    const fetchVenue = async () => {
      try {
        ...
        if (!snapshot.empty) {
          ...
          setVenue({
            id: venueSlug,
            nombre: venueSlug?.replace(/-/g, ' ').toUpperCase() || 'DARK VENUE',
            ciudad: 'Madrid',
            direccion: 'Zona Centro / Chueca',
            fotoUrl: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=1200&auto=format&fit=crop',
            description: 'Experiencia nocturna exclusiva DarkNights.',
            tickets: [
              { id: 'early', nombre: 'Entrada Secret Pass', precio: 12, descripcion: 'Acceso prioritario previo a la 01:00h + 1 Consumición', incluye: ['Sin cola', '1 Copa o 2 Cervezas'], destacado: false },
              { id: 'general', nombre: 'Pase General Dark + Copa', precio: 15, descripcion: 'Acceso total durante toda la noche', incluye: ['Acceso a pista principal', '1 Copa de primera marca'], destacado: true },
              { id: 'vip', nombre: 'Dark VIP Access', precio: 30, descripcion: 'Acceso a zona reservada + 2 Copas Premium + Guardarropa', incluye: ['Pase VIP sin colas', '2 Copas Premium', 'Guardarropa incluido'], destacado: false }
            ]
          });
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white selection:bg-red-500/30 pb-24">
      <div className="relative h-[45vh] min-h-[320px] w-full overflow-hidden">
        <img
          src={venue.fotoUrl || 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=1200&auto=format&fit=crop'}
          alt={venue.nombre}
        ...
        <div className="absolute bottom-8 left-6 right-6 max-w-4xl mx-auto space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/20 border border-red-500/30 text-red-400 text-[10px] font-black uppercase tracking-widest">
            <ShieldCheck className="w-3.5 h-3.5" />
            Muro Oficial DarkNights
          </div>
          ...
          {ticketsList.map((ticket) => (
            <motion.div
              ...
              className={`relative bg-slate-900 border rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-2xl transition-all ${
                ticket.destacado
                  ? 'border-red-500/60 bg-gradient-to-r from-red-950/30 via-slate-900 to-slate-900 shadow-red-900/20'
                  : 'border-white/10'
              }`}
            >
              {ticket.destacado && (
                <span className="absolute -top-3 left-8 bg-gradient-to-r from-red-600 to-amber-600 text-white text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full shadow-lg flex items-center gap-1">
              ...
              <div className="space-y-3 flex-1">
                <div className="flex items-baseline gap-3">
                  ...
                  <span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-amber-500">
                    {ticket.precio}€
                  </span>
                </div>
              ...
              <button
                ...
                className={`w-full md:w-auto px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl flex items-center justify-center gap-2 shrink-0 ${
                  ticket.destacado
                    ? 'bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white shadow-red-500/25'
