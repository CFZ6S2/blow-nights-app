...
function DoorScannerContent() {
  ...
  if (authorized === false) {
    return (
      <div className="min-h-screen bg-[#090a10] text-slate-900 flex flex-col items-center justify-center p-6 text-center">
  ...
  return (
    <div className="min-h-screen bg-[#07080d] text-slate-900 flex flex-col items-center justify-between p-4 font-sans select-none">
      {/* Header */}
      <header className="w-full max-w-sm flex justify-between items-center bg-[#11131f] border border-[#23273f] px-4 py-3 rounded-xl">
        <div>
          ...
          <h1 className="text-sm font-bold text-slate-900 truncate max-w-[200px]">{eventTitle || 'Cargando...'}</h1>
        </div>
      ...
      <div className="w-full max-w-sm flex bg-[#11131f] border border-[#23273f] rounded-lg p-1 my-3">
        <button 
          ...
          className={`flex-1 py-2 text-xs font-bold rounded-md transition-colors ${mode === 'scanner' ? 'bg-[#a855f7] text-slate-900' : 'text-gray-400 hover:text-slate-900'}`}
        >
          Escáner QR
        </button>
        <button 
          onClick={() => setMode('search')} 
          className={`flex-1 py-2 text-xs font-bold rounded-md transition-colors ${mode === 'search' ? 'bg-[#a855f7] text-slate-900' : 'text-gray-400 hover:text-slate-900'}`}
        >
          Buscador Manual
        </button>
      </div>
        ...
        <div className="relative w-full max-w-sm aspect-[3/4] bg-black rounded-2xl overflow-hidden border border-[#2b2f52] shadow-2xl flex items-center justify-center">
          {!cameraStarted ? (
            <div className="flex flex-col items-center justify-center p-6 text-center z-10">
              <div className="w-20 h-20 bg-fuchsia-900/30 rounded-full flex items-center justify-center mb-4 border border-slate-700/50">
                <span className="material-icons text-4xl text-slate-800">photo_camera</span>
              </div>
              <h3 className="text-slate-900 font-bold text-lg mb-2">Permiso de Cámara</h3>
              <p className="text-gray-400 text-xs mb-6 px-4">Toca el botón para activar la cámara de forma segura.</p>
              <button 
                onClick={() => setCameraStarted(true)}
                className="bg-slate-900 hover:bg-fuchsia-500 text-slate-900 font-black text-sm uppercase px-8 py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(192,132,252,0.4)] active:scale-95"
              >
                Activar Cámara
              </button>
            </div>
          ...
          <div className="bg-[#11131f] border border-[#23273f] rounded-2xl w-full p-6 text-center shadow-xl relative">
            <div className="w-full flex justify-between items-center mb-6">
              ...
              <button 
                  ...
                  className="text-[10px] bg-[#1c1f38] hover:bg-fuchsia-900/30 px-3 py-1 rounded-lg text-slate-800 font-bold border border-fuchsia-900 transition-colors"
              >
                  {pinLength} DÍGITOS
              </button>
            </div>
      ...
      {scanResult && (
        <div className={`absolute inset-0 m-auto w-full max-w-sm aspect-[3/4] flex flex-col items-center justify-center p-6 text-center z-20 rounded-2xl shadow-2xl backdrop-blur-md ${
          ...
          {scanResult.name && (
            <p className="text-base font-bold text-slate-900/90 mt-2 bg-black/50 px-4 py-1.5 rounded-lg border border-white/10">
              {scanResult.name}
            </p>
          )}
        </div>
...
export default function DoorScannerPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07080d] text-slate-900 flex items-center justify-center text-xs">Cargando escáner...</div>}>
      <DoorScannerContent />
    </Suspense>
  );
}
