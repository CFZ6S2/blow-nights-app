...
function TicketContent() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-black flex items-center justify-center text-slate-900 text-xs font-mono">{t('ticket.generatingPass')}</div>;
  }

  if (!request || !chill) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-slate-900 p-6">
  ...
  return (
    <div className="min-h-screen bg-black text-slate-900 p-4 flex flex-col items-center">
      
      <div className="w-full max-w-sm mt-8">
        {/* Etiqueta Superior */}
        <div className="bg-gradient-to-r from-slate-800 to-purple-600 p-4 rounded-t-3xl text-center shadow-[0_0_20px_rgba(217,70,239,0.2)]">
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900/80">{t('ticket.digitalAccessPass')}</span>
          <h1 className="text-xl font-black text-slate-900 mt-1">{chill.title}</h1>
          <p className="text-xs text-slate-900/90 mt-1 capitalize">{chill.type === 'commercial' ? t('ticket.privateParty') : t('ticket.socialChill')}</p>
        </div>
          ...
          {hasQRCode ? (
            <>
              ...
              <div className="w-full flex flex-col items-center mb-6">
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">{t('ticket.accessPin')}</p>
                <div className="text-3xl font-black tracking-widest text-slate-900 border border-white/10 bg-white/5 px-6 py-2 rounded-xl">
                  #{generatedPin}
                </div>
              </div>
            ...
            <div className="w-24 h-24 bg-fuchsia-500/10 rounded-full flex items-center justify-center border border-slate-700/30 mb-6">
              <Key className="text-slate-900" size={32} />
            </div>
          )}
          ...
          <div className="w-full space-y-4">
            ...
            <div className="pt-4 border-t border-[#23273f]">
              ...
              <a 
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-3 flex items-center justify-center gap-2 w-full bg-[#1e2238] hover:bg-[#282d4a] border border-[#3b3f66] text-slate-900 py-3 rounded-xl text-xs font-bold transition-all"
              >
        ...
        {!user && (
          <div className="mt-6 bg-gradient-to-r from-purple-900/40 to-indigo-900/40 border border-purple-500/30 rounded-2xl p-5 text-center shadow-lg relative overflow-hidden">
            ...
            <h4 className="text-slate-900 font-bold mb-1">{t('ticket.seeWhoIsInside')}</h4>
            <p className="text-slate-500 text-xs mb-4">{t('ticket.createProfile')}</p>
            <Link 
