...
export default function SetupProfilePage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6 pb-24 selection:bg-fuchsia-500/30">
        ...
        <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-10 shadow-2xl space-y-10">
          <header className="text-center space-y-4">
            <div className="inline-block p-4 rounded-[1.5rem] bg-gradient-to-tr from-fuchsia-500 to-indigo-500 shadow-2xl mb-2">
              <span className="material-icons text-slate-900 text-3xl">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                ...
                <div className="flex flex-col items-center space-y-6">
                  <div className="relative w-40 h-40 group">
                    <div className="w-full h-full rounded-full overflow-hidden border-4 border-white/5 group-hover:border-slate-700 transition-all duration-500 shadow-2xl">
                      {imagePreview ? (
                        <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center bg-white text-slate-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      placeholder="Ej: Marc"
                      className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-slate-900 font-bold outline-none focus:bg-white/[0.08] transition-all"
                    />
                  </div>
                  <div className="space-y-3">
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    ...
                    <select
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-slate-900 font-bold outline-none appearance-none"
                    >
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {[
                      ...
                      >
                        ...
                        <span className="text-[8px] font-black uppercase tracking-widest text-slate-900/60">{m.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-white/5 border border-white/10 rounded-3xl p-5 text-slate-900 font-bold outline-none resize-none transition-all"
                  />
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-3">
                    ...
                    <input type="number" value={altura || ''} onChange={(e) => setAltura(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-slate-900 text-center font-bold" placeholder="180" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Peso</label>
                    <input type="number" value={peso || ''} onChange={(e) => setPeso(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-slate-900 text-center font-bold" placeholder="75" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Cuerpo</label>
                    <select value={complexion || ''} onChange={(e) => setComplexion(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-slate-900 font-bold text-xs appearance-none">
                      <option value="">--</option>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-5 gap-3">
                    ...
                    {extraPhotosPreview.length < 5 && (
                      <div className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-white/5 border-2 border-dashed border-white/10 hover:border-slate-700/50 transition-all flex items-center justify-center cursor-pointer">
          ...
          <div className="flex gap-4 pt-4">
            {currentStep > 1 && (
              <button
                onClick={prevStep}
                className="flex-1 bg-white/5 hover:bg-white/10 text-slate-900 font-black py-6 rounded-[2rem] text-xs uppercase tracking-widest transition-all border border-white/10"
              >
                Atrás
              </button>
            )}
            ...
            {currentStep < totalSteps ? (
              ...
              <button
                ...
                className={`flex-[2] bg-gradient-to-r from-slate-800 to-indigo-600 text-slate-900 font-black py-6 rounded-[2rem] text-xs uppercase tracking-[0.3em] shadow-[0_20px_50px_rgba(192,38,211,0.3)] transition-all ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-95'}`}
              >
