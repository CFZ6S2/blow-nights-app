...
function ClaimVenueContent() {
  ...
  return (
    <div className="min-h-screen bg-[#07080d] text-slate-900 flex flex-col justify-center items-center p-6 selection:bg-amber-500/30">
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-md bg-white border border-white/10 p-8 rounded-3xl relative overflow-hidden shadow-2xl"
      >
        ...
        <p className="text-slate-500 text-sm mb-6 leading-relaxed">
          <Trans i18nKey="business.claim.description">
            Tus promotores (RRPPs) ya están utilizando <strong>DarkNights</strong> para gestionar listas y accesos en tu local de forma independiente.
          </Trans>
        </p>
        ...
        <form onSubmit={handleClaim} className="space-y-4">
          <div>
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1 block">{t('business.claim.form.name_label')}</label>
          ...
          <div>
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1 block">{t('business.claim.form.phone_label')}</label>
...
export default function ClaimVenuePage() {
  const { t } = useTranslation();
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07080d] flex items-center justify-center text-xs text-slate-900">{t('common.loading')}</div>}>
      <ClaimVenueContent />
    </Suspense>
  );
}
