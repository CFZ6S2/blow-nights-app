...
export default function SuperAdminPage() {
  ...
  const ROLE_OPTIONS = [
    ...
    { value: 'superadmin', label: 'Super Admin', icon: 'shield', color: 'text-slate-800', redirect: '/super-admin' },
  ...
  if (loading || !isSuperAdmin) return <div className="min-h-screen bg-slate-50" />;

  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-12 pb-32">
      <header className="mb-8">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-slate-800">{t('super_admin_title')}</h1>
            <p className="text-xs text-slate-500">{t('super_admin_subtitle')}</p>
          </div>
        </div>

        <div className="flex gap-2 overflow-x-auto no-scrollbar border-b border-white/10 pb-2">
          <button onClick={() => setActiveTab('usuarios')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'usuarios' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_users')}</button>
          <button onClick={() => setActiveTab('roles')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'roles' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_roles')}</button>
          <button onClick={() => setActiveTab('ciudades')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'ciudades' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_cities')}</button>
          <button onClick={() => setActiveTab('partners')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'partners' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_partners')}</button>
          <button onClick={() => setActiveTab('facturacion')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'facturacion' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_billing')}</button>
          <button onClick={() => setActiveTab('locales')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'locales' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_venues')}</button>
          <button onClick={() => setActiveTab('eventos')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'eventos' ? 'bg-indigo-600 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_events')}</button>
          <button onClick={() => setActiveTab('rrpps')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'rrpps' ? 'bg-slate-900 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_rrpps')}</button>
          <button onClick={() => setActiveTab('organizadores')} className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors whitespace-nowrap ${activeTab === 'organizadores' ? 'bg-indigo-600 text-slate-900' : 'bg-white/5 text-slate-500'}`}>{t('tab_organizers')}</button>
        </div>
      </header>
      ...
      {activeTab === 'usuarios' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-6">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Buscar Usuarios</h2>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none"
              />
              <button onClick={handleAdminUserSearch} disabled={userSearching} className="bg-slate-900 px-5 py-3 rounded-xl text-sm font-bold hover:bg-fuchsia-500 transition-colors disabled:opacity-50">
            ...
            {userSearchResults.length > 0 ? (
              <div className="space-y-3">
                <p className="text-xs text-slate-800 font-bold uppercase mb-2">Resultados de búsqueda ({userSearchResults.length})</p>
                  ...
                  <div 
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${
                      selectedUserDetail?.id === u.id
                        ? 'bg-fuchsia-500/10 border-slate-700/50'
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <p className="text-xs text-slate-500 font-bold uppercase">Últimos Registros</p>
                  <select
          ...
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl h-[80vh] flex flex-col">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600 mb-6">Detalle de Usuario</h2>
            {!selectedUserDetail ? (
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-sm mb-2">{selectedUserDetail.email}</p>
                    <div className="flex flex-wrap gap-2">
                      <span className="px-2 py-1 bg-white/10 text-slate-900 text-[10px] rounded uppercase font-bold">{selectedUserDetail.role || 'user'}</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-black/30 p-4 rounded-xl border border-white/5">
                    ...
                    <p className="text-xs font-mono text-slate-600 break-all">{selectedUserDetail.id}</p>
                  </div>
                  <div className="bg-black/30 p-4 rounded-xl border border-white/5">
                  ...
                  <div className="bg-black/30 p-4 rounded-xl border border-white/5">
                    ...
                    <p className="text-xs text-slate-600">{selectedUserDetail.phone || 'No registrado'}</p>
                  </div>
                  <div className="bg-black/30 p-4 rounded-xl border border-white/5">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <select
                        ...
                        className="bg-black/50 border border-white/20 rounded-lg px-2 py-1.5 text-xs text-slate-900 outline-none w-full appearance-none"
                      >
                ...
                <div className="space-y-2 mt-8">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-fuchsia-900/20 hover:bg-fuchsia-900/40 text-slate-800 border border-slate-700/20 px-4 py-3 rounded-xl text-sm font-bold transition-colors disabled:opacity-50"
                  >
      ...
      {activeTab === 'roles' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-6">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Asignar Rol por Email</h2>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none"
              />
              <button onClick={handleRoleSearch} disabled={roleSearching} className="bg-slate-900 px-5 py-3 rounded-xl text-sm font-bold hover:bg-fuchsia-500 transition-colors disabled:opacity-50">
                ...
                <div className="bg-black/30 border border-white/10 p-4 rounded-2xl">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-12 h-12 rounded-full bg-fuchsia-500/20 flex items-center justify-center">
                      <span className="material-icons text-slate-800">person</span>
                    </div>
                    <div>
                      <p className="font-bold text-lg">{roleSearchResult.nick || 'Sin nick'}</p>
                      <p className="text-xs text-slate-500">{roleSearchResult.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className={`px-2 py-1 rounded-lg font-bold uppercase tracking-wider ${
                      roleSearchResult.role && roleSearchResult.role !== 'user'
                        ? 'bg-fuchsia-500/20 text-slate-800'
          ...
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4 h-[70vh] flex flex-col">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Usuarios con Rol Asignado ({recentAssignments.length})</h2>
                  ...
                  <div key={u.id} className="bg-black/30 border border-white/5 p-4 rounded-2xl">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <span className={`material-icons ${roleInfo?.color || 'text-slate-500'}`}>{roleInfo?.icon || 'person'}</span>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${roleInfo?.color || 'text-slate-500'} bg-white/5`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-8">
            ...
            {cities.filter(c => c.readyForFranchise).length > 0 && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-gradient-to-r from-red-600 to-orange-500 rounded-3xl p-6 shadow-2xl border-2 border-red-400/50">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none mb-1">¡Territorios Listos para Venta!</h2>
                    ...
                    <div className="space-y-2">
                      {cities.filter(c => c.readyForFranchise).map(c => (
                        <div key={c.id} className="flex justify-between items-center bg-black/20 rounded-xl px-4 py-2 border border-black/10">
                          <span className="font-bold text-slate-900 capitalize">{c.slug || c.id}</span>
            ...
            {applications.length > 0 && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white border-2 border-slate-700/50 rounded-3xl p-6 shadow-2xl">
                <div className="flex items-start gap-4">
                  <span className="text-4xl">💼</span>
                  <div className="w-full">
                    <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none mb-1">Nuevas Solicitudes de City Manager</h2>
                    <p className="text-sm text-slate-500 mb-4">Revisa y contacta a los candidatos para asignarles su plaza.</p>
                    <div className="space-y-3">
                        ...
                        <div key={app.id} className="bg-black/30 rounded-xl p-4 border border-white/10">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              ...
                              <p className="text-xs text-slate-800 font-bold uppercase">{app.cityId === 'other' ? 'Otra Ciudad' : app.cityId}</p>
                            </div>
                            <span className="px-2 py-1 bg-yellow-500/20 text-yellow-500 rounded-md text-xs font-bold uppercase">Pendiente</span>
                          </div>
                          <p className="text-sm text-slate-600 mb-1">📞 {app.phone} | ✉️ {app.email}</p>
                          <p className="text-xs text-slate-500 italic border-l-2 border-slate-700/30 pl-2 mt-2">{app.experience}</p>
                          ...
                          <div className="mt-4 flex gap-2">
                            <button 
                              ...
                              className="px-4 py-2 bg-slate-900 hover:bg-fuchsia-500 rounded-lg text-xs font-bold uppercase transition-colors"
                            >
                              Marcar Revisada
                            </button>
            ...
            <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4">
              <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Activar Nueva Licencia</h2>
            <div className="space-y-3">
              <div className="flex gap-3">
                <input placeholder="City Slug (ej: ibiza)" value={newCitySlug} onChange={e => setNewCitySlug(e.target.value)} className="w-1/2 bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
                <input placeholder="Nombre (ej: Ibiza)" value={newCityName} onChange={e => setNewCityName(e.target.value)} className="w-1/2 bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
              </div>
              
              <div className="flex gap-3">
                <input placeholder="Email del City Manager (Opcional)" type="email" value={newCityManagerEmail} onChange={e => setNewCityManagerEmail(e.target.value)} className="w-1/2 bg-black/30 border border-slate-700/30 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
              ...
              <input placeholder="Empresa / Nombre Partner (Si no hay manager)" value={newCityPartnerName} onChange={e => setNewCityPartnerName(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
              <input placeholder="Email de Facturación / Partner" type="email" value={newCityPartnerEmail} onChange={e => setNewCityPartnerEmail(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
              ...
              <div className="flex items-center gap-3 bg-black/30 px-4 py-3 rounded-xl border border-white/10">
                <span className="text-sm text-slate-500">Canon Anual (€):</span>
              ...
              <button onClick={handleCreateCity} disabled={savingCity} className="w-full bg-slate-900 py-3 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-fuchsia-500 transition-colors mt-2">
          ...
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4 h-[60vh] flex flex-col">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Ciudades Operativas</h2>
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              {cities.map(c => {
                const isMadrid = c.id === 'madrid';
                const isActive = c.license?.status === 'active';
                return (
                  <div key={c.id} className={`bg-black/30 border ${isMadrid ? 'border-slate-700/50' : 'border-white/5'} p-4 rounded-2xl`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-lg">{c.name}</h3>
                          {isMadrid && <span className="bg-fuchsia-500/20 text-slate-800 px-2 py-0.5 rounded text-[10px] font-bold uppercase">Matriz Directa</span>}
                        ...
                        <p className="text-[11px] text-slate-500 mt-1">Partner: <span className="text-slate-900">{c.partner_name || 'Nadie'}</span> ({c.partner_email || 'Sin email'})</p>
                        <p className="text-[10px] text-blue-300 font-mono mt-1">Stripe: {c.partner_stripe_account_id || 'No vinculado'}</p>
                        
                        {!isMadrid && c.license && (
                          <div className="mt-2 bg-white/5 p-2 rounded-lg text-[10px] text-slate-500">
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-4 pt-4 border-t border-white/10">
                        <div className="flex gap-2">
                          <input 
                            ...
                            className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs focus:border-slate-700 outline-none"
                          />
      ...
      {activeTab === 'facturacion' && (
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-fuchsia-900/40 to-black border border-slate-700/30 p-6 rounded-3xl">
              <h3 className="text-xs font-bold uppercase tracking-widest text-slate-800 mb-2">Revenue Global Matriz</h3>
              <p className="text-4xl font-black font-mono">14.250 <span className="text-lg text-slate-800">€</span></p>
              <p className="text-xs text-slate-500 mt-2">Mes actual (Simulación)</p>
            </div>
            <div className="bg-white/5 border border-white/10 p-6 rounded-3xl">
              ...
              <p className="text-[10px] text-slate-500 mt-2">100% Retenido</p>
            </div>
            <div className="bg-white/5 border border-white/10 p-6 rounded-3xl">
              ...
              <p className="text-[10px] text-slate-500 mt-2">Suscripciones (60%) + Entradas (1% + 0.25€)</p>
            </div>
          </div>

          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600 mb-6">Desglose Red de Franquicias</h2>
            <div className="space-y-6">
              {/* Dummy data for visual representation of the billing dashboard */}
              <div>
                <div className="flex justify-between text-xs font-bold mb-2">
                  <span>Madrid (Operación Propia)</span>
                  <span className="font-mono text-slate-800">8.400 €</span>
                </div>
      ...
      {activeTab === 'locales' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Crear Local / Punto</h2>
            <div className="space-y-3">
              <input placeholder="Nombre (ej: Parque del Retiro)" value={newVenueName} onChange={e => setNewVenueName(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
              <select value={newVenueType} onChange={e => setNewVenueType(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none">
              ...
              <div className="flex gap-3">
                <input placeholder="Latitud (ej: 40.4168)" type="number" value={newVenueLat} onChange={e => setNewVenueLat(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
                <input placeholder="Longitud (ej: -3.7038)" type="number" value={newVenueLng} onChange={e => setNewVenueLng(e.target.value)} className="w-full bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none" />
              </div>
              <button onClick={handleCreateVenue} disabled={savingVenue} className="w-full bg-slate-900 py-3 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-fuchsia-500 transition-colors">
                Crear Punto
              </button>
            </div>
          </section>
          ...
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4 h-[60vh] flex flex-col">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Directorio de Locales</h2>
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              {venues.map(v => (
                <div key={v.id} className="bg-black/30 border border-white/5 p-4 rounded-2xl">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 uppercase tracking-widest">{v.type} | Ciudad: {v.cityId}</p>
                  ...
                  {assigningVenueId === v.id && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="mt-4 pt-4 border-t border-white/10">
                      <div className="flex gap-2">
                        <input placeholder="Buscar por nickname..." value={searchNick} onChange={e => setSearchNick(e.target.value)} className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-xs focus:border-slate-700 outline-none" />
          ...
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-100/50">
                <tr>
                  <th className="p-4 font-bold text-slate-500">Evento</th>
                  <th className="p-4 font-bold text-slate-500">Organizador</th>
                  <th className="p-4 font-bold text-slate-500">Fecha</th>
                  <th className="p-4 font-bold text-slate-500">Tickets</th>
                  <th className="p-4 font-bold text-slate-500">Estado</th>
                  <th className="p-4 font-bold text-slate-500">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                {events.map(e => {
                  ...
                  return (
                    <tr key={e.id} className="hover:bg-slate-100/30 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${e.status === 'active' ? 'bg-green-500/10 text-green-400' : 'bg-slate-100 text-slate-500'}`}>
                          {e.status || 'active'}
                        </span>
                      </td>
                      <td className="p-4">
      ...
      {activeTab === 'partners' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Dar de Alta Partner (RRPP)</h2>
            <div className="space-y-3">
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-3 text-sm focus:border-slate-700 outline-none" 
                />
                <button onClick={handleSearchNewPartner} disabled={searchingPartner} className="bg-slate-900 px-4 py-3 rounded-lg text-sm font-bold hover:bg-fuchsia-500 transition-colors">
                  Buscar
                </button>
              </div>
                  ...
                  {partnerSearchResults.map(u => (
                    <div key={u.id} className="flex justify-between items-center bg-white/10 p-3 rounded-xl">
                      ...
                      {!u.isAssociatePartner ? (
                        ...
                        <span className="text-xs text-slate-800 font-bold">Ya es Partner</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <section className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4 h-[60vh] flex flex-col">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Partners Activos</h2>
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 no-scrollbar">
              ...
              {partners.map(p => (
                <div key={p.id} className="bg-black/30 border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">{p.email || 'Sin email'}</p>
                  </div>
      ...
      {activeTab === 'rrpps' && (
        <div className="space-y-8">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white border-2 border-slate-700/50 rounded-3xl p-6 shadow-2xl">
              <div className="flex items-start gap-4">
                <span className="text-4xl">📋</span>
                <div className="w-full">
                  <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none mb-1">Solicitudes RRPP Pendientes ({rrppApplications.length})</h2>
                  <p className="text-sm text-slate-500 mb-4">Revisa y aprueba o rechaza las solicitudes de RRPP independientes.</p>
                  <div className="space-y-3">
                    ...
                    {rrppApplications.map(app => (
                      <div key={app.id} className="bg-black/30 rounded-xl p-4 border border-white/10">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <p className="font-bold text-slate-900 text-lg">{app.nick || 'Sin nick'}</p>
                            <p className="text-xs text-slate-500">{app.email}</p>
                          </div>
                          <span className="px-2 py-1 bg-yellow-500/20 text-yellow-500 rounded-md text-xs font-bold uppercase">Pendiente</span>
                        </div>
                        <p className="text-sm text-slate-600 mb-1">📞 {app.phone} | 📍 {app.city}</p>
                        <div className="mt-4 flex gap-2">
                          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <section className="md:col-span-1 bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4 h-fit">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Alta de RRPP Global</h2>
            <p className="text-xs text-slate-500 mb-4">Crea un enlace de RRPP y asígnalo a cualquier local.</p>
            <div className="space-y-3">
              <input 
                placeholder="Nombre del RRPP..." 
                value={newPromoterName} 
                onChange={e => setNewPromoterName(e.target.value)} 
                className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-3 text-sm focus:border-slate-700 outline-none" 
              />
              <select
              ...
              <button 
                ...
                className="w-full bg-slate-900 px-4 py-3 rounded-lg text-sm font-bold hover:bg-fuchsia-500 transition-colors disabled:opacity-50"
              >
          ...
          <section className="md:col-span-2 bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4 flex flex-col h-[75vh]">
            <div className="flex justify-between items-center">
              <h2 className="text-sm font-black uppercase tracking-widest text-slate-600">Auditoría de Promotores ({promoters.length})</h2>
            </div>
                ...
                <div key={p.id} className="bg-black/30 border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                  <div>
                    <h3 className="font-bold flex items-center gap-2">
                      {p.name}
                      <span className="text-[9px] bg-slate-100 px-2 py-0.5 rounded font-mono text-slate-600">{p.code}</span>
                    </h3>
                    <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest">
                      Local: <span className="text-slate-900">{p.venueId}</span> | Evento: <span className="text-slate-900">{p.eventId}</span>
                    </p>
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase ${p.liquidated_by_venue ? 'bg-blue-900/50 text-blue-400' : 'bg-slate-100 text-slate-500'}`}>
                        {p.liquidated_by_venue ? 'Liquidado' : 'Pendiente Pago'}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2">
                    <button onClick={() => copyPromoterLink(p.code)} className="bg-white/10 text-slate-900 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-white/20 transition-colors uppercase">
                      Copiar Enlace
                    </button>
                    <button onClick={() => handleTogglePromoterStatus(p)} className="bg-white/5 border border-white/10 text-slate-900 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-white/10 transition-colors uppercase">
      ...
      {activeTab === 'organizadores' && (
        <div className="space-y-8">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white border-2 border-indigo-500/50 rounded-3xl p-6 shadow-2xl">
              <div className="flex items-start gap-4">
                <span className="text-4xl">🎪</span>
                <div className="w-full">
                  <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none mb-1">Solicitudes de Organizadores Pendientes ({organizerApps.length})</h2>
                  <p className="text-sm text-slate-500 mb-4">Revisa y aprueba o rechaza las solicitudes para organizar eventos independientes.</p>
                  <div className="space-y-3">
