...
export default function AdminDashboard() {
  ...
  if (loading || !isAdmin) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-slate-700"></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 pb-24">
      ...
      <header className="mb-8">
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            {isSuperAdmin && (
              <button onClick={() => router.push('/super-admin')} className="bg-slate-900/20 text-slate-800 border border-fuchsia-600/50 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-slate-900/30 transition-all">
                Panel Maestro
              </button>
            )}
            <button onClick={() => router.push('/')} className="bg-white/5 text-slate-500 border border-white/10 px-4 py-2 rounded-xl text-xs font-bold hover:bg-white/10 transition-all">
              Volver
            </button>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-300/50 rounded-2xl p-4 mb-8 shadow-xl">
        <h2 className="text-lg font-black text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2">
          <span className="material-icons text-slate-900">settings_input_component</span>
          Ecosistema
        </h2>
        ...
        <div className="flex items-center justify-between p-4 bg-black/50 rounded-xl border border-white/5">
          <div>
            ...
            <p className="text-xs text-slate-500">{t('admin.dark_nights_desc')}</p>
          </div>
          <button
            onClick={async () => {
            ...
            className={`w-14 h-8 rounded-full relative transition-colors duration-300 ${platformSettings?.enableDarkNightsBridge ? 'bg-slate-900' : 'bg-slate-700'}`}
          >
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <StatCard label={t('admin.users')} value={stats.totalUsers} icon="people" color="from-fuchsia-500/20 to-fuchsia-500/5" border="border-slate-700/20" />
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'rrpps' && (
          <motion.div key="rrpps" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            ...
            {rrppApps.map(app => (
              <div key={app.id} className="bg-white/5 backdrop-blur-md border border-slate-700/30 rounded-2xl p-4">
          ...
          <motion.div key="stats" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <span className="material-icons text-slate-800">trending_up</span>
                Ratios del Sistema
              </h2>
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-white/50 rounded-2xl p-4 border border-white/5 text-center">
          ...
          <motion.div key="users" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 focus:outline-none focus:border-slate-700/50 transition-colors"
              />
            </div>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    <button 
                      ...
                      className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all ${
                        u.premium ? 'bg-amber-500/20 text-amber-500 border border-amber-500/30' : 'bg-white/5 text-slate-500 border border-white/10'
                      }`}
                    >
            ...
            {lastVisible && (
              <button 
                ...
                className="w-full py-3 bg-white/5 hover:bg-white/10 rounded-2xl text-xs font-bold text-slate-600 border border-white/10 transition-all"
              >
...
function TabButton({ active, onClick, label, icon }: { active: boolean, onClick: () => void, label: string, icon: string }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2 rounded-2xl text-sm font-bold transition-all whitespace-nowrap ${active ? 'bg-slate-900 text-slate-900 shadow-lg' : 'bg-white/5 text-slate-500 border border-white/5'}`}>
