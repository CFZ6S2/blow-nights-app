...
function BuyTicketContent() {
  ...
  if (!venueId || !eventId) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-xl font-bold text-slate-900 mb-2">{t('buy.invalid_link.title')}</h1>
        <p className="text-slate-500">{t('buy.invalid_link.description')}</p>
      </div>
    );
  }
  ...
  if (tiers.length === 0) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-xl font-black text-slate-900 mb-2">{t('buy.sold_out.title')}</h1>
  ...
  return (
    <div className="min-h-screen bg-black text-slate-900 p-6 flex flex-col items-center justify-center">
      <div className="bg-white/80 border border-white/5 p-8 rounded-3xl w-full max-w-md shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 bg-slate-900 text-slate-900 text-[10px] font-black px-4 py-1 rounded-bl-2xl uppercase tracking-widest">
          {t('buy.secure_purchase')}
        </div>

        <h1 className="text-2xl font-black mb-1 mt-4 text-slate-800 uppercase tracking-widest">
          {venueName || t('buy.buy_ticket')}
        </h1>
        {eventTitle && <p className="text-slate-900 font-bold text-sm mb-1">{eventTitle}</p>}
        ...
        <div className="bg-white/5 rounded-2xl p-4 mb-6 border border-white/5">
          ...
          <div className="border-t border-white/10 pt-2 mt-2 flex justify-between">
            ...
            <span className="font-black text-slate-800 text-xl">{totalPrice.toFixed(2)} €</span>
          </div>
        </div>
