...
export default function CityManagerDashboard() {
  ...
  if (authLoading || dataLoading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><div className="w-8 h-8 border-4 border-slate-700 border-t-transparent rounded-full animate-spin" /></div>;

  if (!cityData) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        <AlertTriangle className="w-16 h-16 text-yellow-500 mb-4" />
        <h1 className="text-2xl font-bold text-slate-900 mb-2">{t('city_manager_access_denied')}</h1>
        <p className="text-slate-500 max-w-md">{t('city_manager_no_franchise')}</p>
        <button onClick={() => router.push('/')} className="mt-8 px-6 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-slate-900 font-bold transition-colors">
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 selection:bg-fuchsia-500/30 pb-20">
        ...
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-slate-500 font-bold uppercase tracking-widest">{cityData.name || cityData.id}</span>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* Facturación Global */}
            <div className="bg-white border border-slate-200 p-6 rounded-3xl relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                <TrendingUp className="w-24 h-24" />
              </div>
              <p className="text-sm font-bold text-slate-500 mb-1">Facturación SaaS (Mes)</p>
              <p className="text-4xl font-black text-slate-900">{metrics.monthlyGrossRevenue} €</p>
              <div className="mt-4 pt-4 border-t border-slate-200/50 flex items-center text-xs text-slate-500 font-medium">
                Generado orgánicamente por los locales de tu zona.
              </div>
            </div>

            {/* Comisión del Manager */}
            <div className="bg-gradient-to-br from-fuchsia-900/40 to-indigo-900/40 border border-slate-700/20 p-6 rounded-3xl relative overflow-hidden group shadow-[0_0_30px_-10px_rgba(168,85,247,0.2)]">
              <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                <span className="text-8xl font-black">€</span>
              </div>
              <p className="text-sm font-bold text-fuchsia-300 mb-1">Tu Comisión Neta (50%)</p>
              <p className="text-4xl font-black text-slate-900">{metrics.managerPayoutDue.toFixed(2)} €</p>
              <div className="mt-4 pt-4 border-t border-slate-700/20 flex items-center text-xs text-fuchsia-200/50 font-medium">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-3xl relative overflow-hidden group">
              ...
              <p className="text-sm font-bold text-slate-500 mb-1">Tickets Vendidos</p>
              <p className="text-4xl font-black text-slate-900">{metrics.totalTickets}</p>
              <div className="mt-4 pt-4 border-t border-slate-200/50 flex items-center text-xs text-slate-500 font-medium">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-4">
            ...
            <button 
              onClick={() => {
                navigator.clipboard.writeText(`https://darknights.com/business?city=${cityData.id}&ref=${user?.uid}`);
                alert('Enlace de afiliado copiado al portapapeles.');
              }}
              className="text-xs bg-fuchsia-500/10 hover:bg-fuchsia-500/20 text-slate-800 border border-slate-700/20 px-4 py-2 rounded-lg font-bold transition-colors flex items-center gap-2"
            >
          ...
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden">
            {venues.length === 0 ? (
              ...
              <div className="divide-y divide-slate-800/50">
                {venues.map(v => (
                  <div key={v.id} className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-100/30 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{v.type === 'club' ? 'Discoteca' : 'Bar/Pub'} • Creado el {new Date(v.createdAt?.toDate?.() || Date.now()).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${v.isActive ? 'bg-green-500/10 text-green-400' : 'bg-slate-100 text-slate-500'}`}>
                        {v.isActive ? 'Operativo' : 'Inactivo'}
                      </span>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${v.subscriptionTier === 'ticketing' ? 'bg-indigo-500/10 text-indigo-400' : v.subscriptionTier === 'promo' ? 'bg-fuchsia-500/10 text-slate-800' : v.subscriptionTier === 'basico' ? 'bg-blue-500/10 text-blue-400' : 'bg-slate-100 text-slate-500'}`}>
        ...
        <section className="mb-12">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden">
            {events.length === 0 ? (
              ...
              <div className="divide-y divide-slate-800/50">
                {events.map(e => (
                  <div key={e.id} className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-100/30 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-slate-100 rounded-xl overflow-hidden flex-shrink-0">
                        {e.banner_url ? (
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${e.status === 'active' ? 'bg-green-500/10 text-green-400' : 'bg-slate-100 text-slate-500'}`}>
