...
function EventPromoters({ venueId, eventId }: { venueId: string; eventId: string }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-white/5 space-y-4">
      <h4 className="text-sm font-bold text-slate-800">Equipo RRPP</h4>
      ...
      <div className="flex gap-2">
        <input 
          ...
          className="flex-1 bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-slate-700"
        />
        <button onClick={handleAddPromoter} className="bg-slate-900 text-slate-900 px-4 py-2 rounded-xl text-sm font-bold">
          Añadir
        </button>
      </div>
            ...
            return (
              <div key={p.id} className={`flex items-center justify-between p-3 rounded-xl border ${p.is_closed ? 'bg-black/40 border-yellow-500/20' : 'bg-black/20 border-white/10'}`}>
                <div>
                  ...
                  {!p.is_closed && (
                    <button onClick={() => copyLink(p.access_token)} className="text-[10px] text-slate-800 hover:text-fuchsia-300 flex items-center mt-1">
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="px-4 py-2 bg-slate-900 text-slate-900 rounded-full text-xs font-bold uppercase tracking-widest"
        >
      ...
      {isCreating && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-1">{t('event_title')}</label>
              <input 
                type="text" 
                value={title} 
                onChange={e => setTitle(e.target.value)}
                className="w-full bg-white border border-white/10 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-700"
                placeholder="Ej. Sábado Loco - Winter Edition"
              />
            </div>
            
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-1">{t('start_date_time')}</label>
              <input 
                type="datetime-local" 
                value={startDate} 
            ...
            <div>
              ...
              <input 
                ...
                className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-slate-900 file:text-slate-900 hover:file:bg-fuchsia-700"
              />
              {flyerPreview && (
            ...
            <div className="pt-4 border-t border-white/10 space-y-4">
              <div className="flex justify-between items-center">
                <h4 className="font-bold text-slate-800">Tipos de Entrada (Tiers)</h4>
                <button onClick={addTier} className="text-xs text-slate-800 border border-fuchsia-400 px-3 py-1 rounded-full">+ Añadir</button>
              </div>
              
              {tiers.map((tier, index) => (
                <div key={tier.id} className="bg-white p-4 rounded-2xl border border-white/5 space-y-3 relative">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Nombre</label>
                      <input type="text" value={tier.name} onChange={e => updateTier(index, 'name', e.target.value)} className="w-full bg-transparent border-b border-white/10 focus:border-slate-700 pb-1 text-sm outline-none" placeholder="Ej. General" />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Precio (€)</label>
                      <input type="number" value={tier.price} onChange={e => updateTier(index, 'price', parseFloat(e.target.value))} className="w-full bg-transparent border-b border-white/10 focus:border-slate-700 pb-1 text-sm outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Cupo (Aforo)</label>
                      <input type="number" value={tier.quota} onChange={e => updateTier(index, 'quota', parseInt(e.target.value))} className="w-full bg-transparent border-b border-white/10 focus:border-slate-700 pb-1 text-sm outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Perks (Ej. 1 Copa)</label>
                      <input type="text" value={tier.perks} onChange={e => updateTier(index, 'perks', e.target.value)} className="w-full bg-transparent border-b border-white/10 focus:border-slate-700 pb-1 text-sm outline-none" />
                    </div>
                  </div>
                  {tiers.length > 1 && (
            ...
            <button
              ...
              disabled={saving}
              className="w-full py-4 bg-slate-900 hover:bg-fuchsia-500 text-slate-900 rounded-2xl font-black uppercase tracking-widest transition-colors mt-6"
            >
        ...
        {events.map(evt => (
          <div key={evt.id} className="bg-white/5 border border-white/10 rounded-3xl p-5 flex flex-col md:flex-row gap-5">
            {evt.banner_url ? (
              ...
              <div className="w-full md:w-32 h-32 bg-white rounded-2xl flex items-center justify-center">
            ...
            <div className="flex-1 space-y-2">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  <div className="text-2xl font-black text-slate-800">{evt.stats?.total_sold || 0}</div>
                  <div className="text-[9px] uppercase tracking-widest text-slate-500">{t('sold_tickets')}</div>
                </div>
              </div>

              <div className="pt-3 border-t border-white/5">
                <div className="flex items-center justify-between bg-black/40 p-3 rounded-xl border border-slate-700/20">
                  <div>
                    <div className="text-[10px] text-slate-800 font-bold uppercase tracking-widest">Enlace de Portero (Magic Link)</div>
                    <div className="text-xs text-slate-500 font-mono mt-1">/door/{venueId}/{evt.id}?token={evt.door_access_token.substring(0,8)}...</div>
                  </div>
                  <button 
                    onClick={() => copyMagicLink(evt.id, evt.door_access_token)}
                    className="w-10 h-10 bg-slate-900 rounded-full flex items-center justify-center hover:scale-105 transition-transform"
