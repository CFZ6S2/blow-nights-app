...
export default function MatchOverlay({ matchData, onClose }: { matchData: any, onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[250] bg-slate-50/98 backdrop-blur-[50px] flex flex-col items-center justify-center p-8 text-center overflow-hidden"
    >
      ...
      <motion.div 
        ...
        className={`absolute w-[600px] h-[600px] rounded-full blur-[150px] -z-20 ${isSuper ? 'bg-yellow-500/30' : 'bg-slate-900/30'}`}
      />
      ...
      >
        ...
        >
          ...
          <p className="text-slate-900/60 font-black tracking-[0.5em] uppercase text-[10px] mb-16">{t('match.connected')}</p>
        </motion.div>
        ...
        <div className="flex items-center justify-center mb-24 relative">
          ...
          <motion.div 
            ...
            className={`absolute w-48 h-48 rounded-full border-2 ${isSuper ? 'border-yellow-500' : 'border-slate-700'}`}
          />
          ...
          >
            <motion.span 
              ...
              className="material-icons text-slate-900 text-5xl"
            >
        ...
        >
          ...
          <button 
            onClick={onClose}
            className="w-full py-4 rounded-[2rem] text-slate-900/40 font-black text-[10px] uppercase tracking-[0.4em] hover:text-slate-900/80 transition-all"
          >
            {t('match.maybe_later')}
          </button>
        </motion.div>
