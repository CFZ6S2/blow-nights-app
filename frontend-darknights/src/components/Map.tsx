    ...
    >
      ...
      >
        ...
        <div className={`w-12 h-12 rounded-full border-2 ${
          ...
          : u.premium ? 'border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.4)]' : 'border-slate-700 shadow-[0_0_15px_rgba(217,70,239,0.4)]'
        ...
        {u.verified && (
          <motion.div 
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute -bottom-1 -left-1 bg-blue-500 text-slate-900 p-0.5 rounded-full shadow-lg border-2 border-slate-950 z-20 flex items-center justify-center"
          >
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 hidden group-hover:block z-50 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <motion.div 
            ...
            className="bg-white/90 backdrop-blur-xl border border-white/20 p-4 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] min-w-[160px]"
          >
            <div className="flex items-center gap-3 mb-2">
              <p className="font-black text-base text-slate-900">{u.nick}, {u.edad || '?'}</p>
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 rounded-lg bg-white/5 text-[10px] text-slate-600 font-bold capitalize">{u.rol}</span>
              <span className="px-2 py-1 rounded-lg bg-fuchsia-500/10 text-[10px] text-slate-800 font-bold capitalize">{u.intencion}</span>
            </div>
            <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-center">
              <p className="text-[10px] text-slate-900/40 font-black uppercase tracking-[0.2em]">Ver Perfil</p>
            </div>
          </motion.div>
          <div className="w-3 h-3 bg-white/90 rotate-45 absolute -bottom-1.5 left-1/2 -translate-x-1/2 border-r border-b border-white/20"></div>
        </div>
      </motion.div>
    </Marker>
  );
...
const VenueMarker = memo(({ v, onClick, index }: { v: any, onClick: (id: string) => void, index: number }) => {
  ...
  // Style mapping
  let bgClass = 'bg-white border-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.4)]';
  let iconName = 'place';
  let gradientClass = 'bg-indigo-500/20';
  
  if (v.isClaimed === false) {
    bgClass = 'bg-white border-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.5)]';
    iconName = 'storefront';
    gradientClass = 'bg-yellow-500/20';
  } else if (v.type === 'public_cruising' || v.type === 'cruising') {
    bgClass = 'bg-white border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.5)]';
    iconName = 'park';
    gradientClass = 'bg-emerald-500/20';
  } else if (v.type === 'sauna') {
    bgClass = 'bg-white border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.5)]';
    iconName = 'hot_tub';
    gradientClass = 'bg-red-500/20';
  } else if (v.type === 'cruising_bar') {
    bgClass = 'bg-white border-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.5)]';
    iconName = 'local_bar';
    gradientClass = 'bg-orange-500/20';
  } else if (v.type === 'club' || v.type === 'nightlife') {
    bgClass = 'bg-white border-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.5)]';
    iconName = 'nightlife';
    ...
    >
      ...
      >
        ...
        <div className={`w-12 h-12 rounded-[1rem] border-2 flex items-center justify-center shadow-2xl transform group-hover:scale-110 group-active:scale-95 transition-all duration-300 ${
          isBoosted ? 'bg-white border-yellow-400 shadow-[0_0_25px_rgba(250,204,21,0.6)]' : bgClass
        }`}>
          {v.coverImage ? (
            <img src={v.coverImage} alt={v.name} className="w-full h-full object-cover rounded-[14px] opacity-80" />
          ) : (
            <span className="material-icons text-slate-900 text-2xl">
              {iconName}
            </span>
          )}
        </div>
        ...
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block z-50 animate-in fade-in slide-in-from-bottom-2 duration-300 pointer-events-none">
          <div className="bg-white/90 backdrop-blur-xl border border-white/20 px-4 py-2 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] whitespace-nowrap flex flex-col items-center">
            ...
            {v.isClaimed === false ? (
              ...
              <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{v.type}</p>
            )}
          </div>
          <div className="w-3 h-3 bg-white/90 rotate-45 absolute -bottom-1.5 left-1/2 -translate-x-1/2 border-r border-b border-white/20"></div>
        </div>
  ...
  return (
    <Marker latitude={c.approx_lat} longitude={c.approx_lng} anchor="center">
      ...
      >
        ...
        <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${isCommercial ? 'from-amber-400 to-orange-500 border-amber-300 shadow-[0_0_20px_rgba(245,158,11,0.5)]' : 'from-slate-800 to-purple-700 border-fuchsia-400 shadow-[0_0_20px_rgba(192,38,211,0.5)]'} border-2 flex items-center justify-center group-hover:scale-110 transition-transform`}>
          <span className="material-icons text-slate-900 text-xl">{isCommercial ? 'vpn_key' : 'local_fire_department'}</span>
        </div>
        <div className={`absolute -bottom-1 -right-1 bg-black border ${isCommercial ? 'border-amber-500/50 text-amber-400' : 'border-slate-700/50 text-slate-800'} rounded-full px-1.5 py-0.5 text-[8px] font-black`}>
          {spots > 0 ? spots : '0'}
        </div>
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 hidden group-hover:block z-50 pointer-events-none">
          <div className={`bg-white/90 backdrop-blur-xl border ${isCommercial ? 'border-amber-500/30' : 'border-slate-700/30'} px-4 py-2 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] whitespace-nowrap flex flex-col items-center`}>
            <p className="font-black text-sm text-slate-900">{c.title}</p>
            <p className={`text-[9px] font-bold ${isCommercial ? 'text-amber-400' : 'text-slate-800'}`}>
              {isCommercial ? 'After Privado' : 'Chill Social'} &middot; {spots > 0 ? `${spots} plazas` : 'Lleno'}
            </p>
          </div>
          <div className={`w-3 h-3 bg-white/90 rotate-45 absolute -bottom-1.5 left-1/2 -translate-x-1/2 border-r border-b ${isCommercial ? 'border-amber-500/30' : 'border-slate-700/30'}`}></div>
        </div>
      </motion.div>
    </Marker>
  );
...
export default function MainMap() {
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      <div className="relative w-full h-[65svh] md:h-[65vh] rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl bg-slate-50">
      <motion.div 
        animate={{ 
        ...
        {isAddingPin && (
          <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center z-30">
            ...
            <motion.div 
              ...
              className="absolute bottom-8 w-[90%] max-w-sm bg-white/95 backdrop-blur-2xl border border-white/10 p-5 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] pointer-events-auto"
            >
              <h3 className="text-sm font-black text-slate-900 mb-4 tracking-tight">{t('map.create_pin', 'Crear Punto Comunitario')}</h3>
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  
                  <button 
                    onClick={() => setNewPinType('sauna')} 
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPinType === 'sauna' ? 'bg-red-600 text-slate-900 shadow-lg shadow-red-500/20' : 'bg-black/40 text-slate-500'}`}
                  >
                    Sauna ♨️
                  </button>
                  <button 
                    onClick={() => setNewPinType('bar')} 
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPinType === 'bar' ? 'bg-orange-600 text-slate-900 shadow-lg shadow-orange-500/20' : 'bg-black/40 text-slate-500'}`}
                  >
                    Bar 🍸
                  </button>
                  <button 
                    onClick={() => setNewPinType('club')} 
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${newPinType === 'club' ? 'bg-cyan-600 text-slate-900 shadow-lg shadow-cyan-500/20' : 'bg-black/40 text-slate-500'}`}
                  >
                    Discoteca 🪩
                  </button>
                </div>
                <input 
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-slate-700 outline-none text-slate-900"
                />
                <button 
          ...
          <div className="absolute bottom-4 left-4 right-4 z-20 flex items-end justify-between">
            ...
            <motion.button
              ...
              className={`w-14 h-14 rounded-2xl backdrop-blur-xl border flex items-center justify-center shadow-lg transition-all ${
                isAvailable
                  ? 'bg-slate-900/80 border-fuchsia-400 shadow-[0_0_20px_rgba(217,70,239,0.4)]'
                  : 'bg-white/60 border-white/10'
              }`}
            >
            ...
            <div className="flex items-center gap-2">
              {profile?.premium && (
                <motion.button
                  ...
                  className={`w-12 h-12 rounded-2xl backdrop-blur-xl text-slate-900 border flex items-center justify-center shadow-lg transition-all ${
                    isAddingPin ? 'bg-slate-900 border-fuchsia-400' : 'bg-white/60 border-white/10 hover:bg-white/10'
                  }`}
                >
              ...
              <motion.button
                ...
                className="w-12 h-12 rounded-2xl bg-white/60 backdrop-blur-xl text-slate-900 border border-white/10 flex items-center justify-center shadow-lg hover:bg-white/10 transition-all"
              >
                <span className="material-icons text-xl text-slate-800">my_location</span>
              </motion.button>

              <motion.button
                whileTap={{ scale: 0.9 }}
                onClick={() => { triggerHaptic(10); setFiltersOpen(true); }}
                className="relative w-12 h-12 rounded-2xl bg-white/60 backdrop-blur-xl text-slate-900 border border-white/10 flex items-center justify-center shadow-lg hover:bg-white/10 transition-all"
              >
                <span className="material-icons text-xl text-slate-800">tune</span>
        ...
        {filtersOpen && (
          ...
          >
            <motion.div 
              ...
              className="w-full max-w-lg bg-white rounded-[2.5rem] p-6 space-y-6 shadow-[0_-20px_80px_rgba(0,0,0,0.8)] border border-white/10 max-h-[85svh] overflow-y-auto"
              onClick={e => e.stopPropagation()}
            >
              <div className="flex justify-between items-center pb-2">
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">{t('map.refine', 'Refinar Búsqueda')}</h3>
                <button 
              ...
              <div className="space-y-4">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-3 flex items-center gap-3 focus-within:border-slate-700 transition-colors">
                  ...
                  <input 
                    ...
                    className="bg-transparent text-slate-900 w-full outline-none text-sm placeholder-slate-500"
                  />
                </div>
                ...
                <div className="flex bg-black/40 border border-white/10 rounded-xl p-1">
                  <button 
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-[10px] uppercase font-black transition-all flex items-center justify-center gap-1 ${radarFilter.mode === 'places' ? 'bg-slate-900 text-slate-900 shadow-lg' : 'text-slate-500 hover:text-slate-900'}`}
                  >
                    <span>📍</span> {t('map.mode_places', 'Lugares')}
                  </button>
                  <button 
                    onClick={() => setRadarFilter(prev => ({ ...prev, mode: 'users' }))}
                    className={`flex-1 py-2.5 rounded-lg text-[10px] uppercase font-black transition-all flex items-center justify-center gap-1 ${radarFilter.mode === 'users' ? 'bg-slate-900 text-slate-900 shadow-lg' : 'text-slate-500 hover:text-slate-900'}`}
                  >
                    <span>👥</span> {t('map.mode_map', 'Mapa')}
                  </button>
                  <button 
                    onClick={() => setRadarFilter(prev => ({ ...prev, mode: 'swipe' }))}
                    className={`flex-1 py-2.5 rounded-lg text-[10px] uppercase font-black transition-all flex items-center justify-center gap-1 ${radarFilter.mode === 'swipe' ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-slate-900 shadow-[0_0_15px_rgba(245,158,11,0.5)]' : 'text-slate-500 hover:text-slate-900'}`}
                  >
              ...
              {radarFilter.mode === 'places' ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {PLACE_CATEGORY_KEYS.map(cat => (
                      <button 
                        ...
                        className={`py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${radarFilter.type === cat.id ? "bg-fuchsia-500 text-slate-900 shadow-xl shadow-fuchsia-500/20" : "bg-white/5 text-slate-500 border border-white/5 hover:bg-white/10"}`}
                      >
                ...
                <div className="flex justify-between items-end">
                  ...
                  <p className="text-slate-800 font-black text-lg">{ageRange[0]} - {ageRange[1]}</p>
                </div>
                <div className="space-y-4">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  ...
                  <p className="text-slate-800 font-black text-lg">{distanceFilter} km</p>
                </div>
                <div className="space-y-4">
                  <input 
                    type="range" 
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {["activo", "pasivo", "versátil"].map((r) => (
                      <button key={r} onClick={() => { triggerHaptic(10); setRoleFilter(roleFilter === r ? null : r); }} className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${roleFilter === r ? "bg-fuchsia-500 text-slate-900 shadow-xl shadow-fuchsia-500/20" : "bg-white/5 text-slate-500 border border-white/5"}`}>{r}</button>
                    ))}
                  </div>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {["conocer", "quedar"].map((i) => (
                      <button key={i} onClick={() => { triggerHaptic(10); setIntentionFilter(intentionFilter === i ? null : i); }} className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${intentionFilter === i ? "bg-fuchsia-500 text-slate-900 shadow-xl shadow-fuchsia-500/20" : "bg-white/5 text-slate-500 border border-white/5"}`}>{i}</button>
                    ))}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <button 
                      ...
                      className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                        onlyWithPhoto 
                        ? 'bg-fuchsia-500/20 border-slate-700/50 text-slate-800' 
