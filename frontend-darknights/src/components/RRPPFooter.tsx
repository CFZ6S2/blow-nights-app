...
export default function RRPPFooter({ saldoQRs = 0, unreadMessages = 0 }: RRPPFooterProps) {
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-50 bg-slate-50/95 backdrop-blur-md border-t border-slate-200 px-4 py-2">
      <div className="max-w-md mx-auto grid grid-cols-4 gap-1">
        ...
        <Link
          ...
          className={`flex flex-col items-center justify-center py-1.5 rounded-2xl transition-all ${
            pathname === '/rrpp' || pathname.includes('/dashboard')
              ? 'text-slate-800 font-bold bg-fuchsia-500/10'
        ...
        >
          <div className="relative">
            <span className="text-lg">💬</span>
            {unread > 0 && (
              <span className="absolute -top-1 -right-2 bg-fuchsia-500 text-slate-900 text-[9px] font-black px-1 rounded-full animate-pulse">
                {unread}
              </span>
            )}
          </div>
