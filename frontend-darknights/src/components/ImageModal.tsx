      ...
      {isOpen && (
        ...
        >
          <motion.div 
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="absolute inset-0 bg-slate-50/90 backdrop-blur-2xl"
          />
          ...
          >
            ...
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={onClose}
              className="absolute -top-12 right-0 w-10 h-10 rounded-full bg-white/10 flex items-center justify-center border border-white/10 hover:bg-white/20 transition-all"
            >
              <span className="material-icons text-slate-900">close</span>
            </motion.button>
          </motion.div>
        </motion.div>
      )}
