  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-white/10 transition-all"
      >
        <span className="material-icons text-sm text-slate-800">location_city</span>
        {currentCity.name}
        ...
        {open && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md bg-white border border-white/10 rounded-[2.5rem] p-6 pb-[env(safe-area-inset-bottom)] space-y-4"
            >
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {cities.map((city) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all ${
                      currentCity.id === city.id
                        ? 'bg-fuchsia-500/10 border border-slate-700/20'
                  ...
                  >
                    ...
                    {currentCity.id === city.id && (
                      <span className="material-icons text-slate-800 text-sm">check_circle</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <button
                onClick={() => setOpen(false)}
                className="w-full py-4 rounded-2xl bg-white/5 text-slate-500 font-black uppercase tracking-[0.2em] text-xs hover:bg-white/10 transition-all"
              >
                {t('city.close')}
              </button>
            </motion.div>
