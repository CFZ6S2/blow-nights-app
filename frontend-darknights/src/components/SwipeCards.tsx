  ...
  if (pingLimitExceeded) {
    return (
      <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-black/80 backdrop-blur-md rounded-[2.5rem]">
        <div className="bg-white border border-slate-300 p-8 rounded-3xl text-center shadow-2xl w-full max-w-sm">
          <div className="text-4xl mb-4">🔥</div>
          <h2 className="text-lg font-black text-slate-900 uppercase leading-tight mb-2">{t('swipe.limit_title')}</h2>
          <p className="text-slate-500 text-xs font-medium mb-6">{t('swipe.limit_desc')}</p>
          
          <div className="space-y-3">
            <button 
              onClick={() => handleCheckout('ping_pack')}
              disabled={isProcessingPayment}
              className="w-full bg-slate-100 text-slate-900 font-bold text-sm tracking-wide py-4 rounded-xl hover:bg-slate-700 transition-all flex items-center justify-between px-4 border border-slate-600"
            >
            ...
            <button 
              ...
              disabled={isProcessingPayment}
              className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-slate-900 font-black text-sm tracking-wide py-4 rounded-xl hover:scale-105 transition-all shadow-[0_0_15px_rgba(245,158,11,0.5)] flex items-center justify-between px-4"
            >
        ...
        {cards.length === 0 ? (
          ...
          cards.map((c, index) => {
            ...
            return (
              <motion.div
                key={c.id}
                className="absolute w-full max-w-[340px] aspect-[3/4] rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-white"
                style={{
              ...
              >
                ...
                {/* User Info */}
                <div className="absolute bottom-24 left-0 w-full p-6 text-slate-900 space-y-1 pointer-events-none">
                  <h2 className="text-3xl font-black">{c.nick}, {c.edad || '?'}</h2>
                  <div className="flex gap-2 text-[10px] font-black uppercase tracking-widest text-slate-600">
                    <span>
                ...
                <div className="absolute bottom-6 w-full flex justify-center items-center gap-6 z-20">
                  <button 
                    ...
                    className="w-14 h-14 rounded-full bg-slate-100/80 border border-slate-600 flex items-center justify-center hover:scale-110 hover:bg-slate-700 transition-all shadow-lg"
                  >
                  ...
                  >
                    <span className="material-icons text-slate-900 text-3xl">local_fire_department</span>
                    <span className="text-[8px] font-black uppercase text-slate-900 tracking-widest mt-1">{t('swipe.come_here')}</span>
                  </button>
                  <button 
