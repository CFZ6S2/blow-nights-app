...
export default function ProfileOverlay({ id, onClose }: ProfileOverlayProps) {
  ...
  if (loading) return (
    <div className="fixed inset-0 z-[100] bg-slate-50/50 backdrop-blur-md flex items-center justify-center">
      <ProfileSkeleton />
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        className="w-full max-w-md h-full bg-slate-50 relative shadow-2xl overflow-y-auto no-scrollbar"
        ref={containerRef}
      >
        {/* Parallax Header */}
        <motion.div 
              ...
              return (
                <div key={i} className="snap-center min-w-full h-full relative overflow-hidden">
                  ...
                  {blurNsfw && !locked && (
                    ...
                    >
                      <span className="material-icons text-slate-900/80 text-3xl">visibility_off</span>
                      <span className="text-[9px] text-slate-900/70 font-black uppercase tracking-widest mt-2">{t('overlay.nsfw_reveal')}</span>
                    </button>
                  )}
                  {locked && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-black/20">
                      ...
                      <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em] bg-black/40 px-4 py-2 rounded-full backdrop-blur-md border border-white/10">{t('overlay.private_album')}</p>
                    </div>
                  )}
            ...
            {showLikeAnim && (
              ...
              >
                <div className="relative">
                  <span className="material-icons text-[120px] text-slate-900 shadow-2xl drop-shadow-[0_0_30px_rgba(217,70,239,0.8)]">favorite</span>
                  {[...Array(12)].map((_, i) => (
                    <motion.div
                      key={i}
        ...
        >
          <div className="bg-slate-50/80 backdrop-blur-2xl border-t border-white/5 rounded-t-[3rem] p-8 shadow-[0_-20px_40px_rgba(0,0,0,0.5)] pointer-events-auto">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('overlay.distance', { distance })}</p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              <div className="px-4 py-2 rounded-xl bg-white/5 border border-white/5 text-[10px] font-bold text-slate-600 uppercase tracking-wider">{profile.rol === 'party_only' ? t('overlay.party_only') : profile.rol}</div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.intereses && profile.intereses.map((interes: string, idx: number) => (
                    <span key={idx} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-sm font-medium text-slate-600">
                      {interes}
                    </span>
                  ))}
                </div>
        ...
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-[110]">
          <div className="bg-white/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-3 flex gap-3 shadow-[0_25px_60px_rgba(0,0,0,0.8)]">
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleLike(false)} className={`flex-1 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all ${hasLiked ? 'bg-slate-100 text-slate-500' : 'bg-gradient-to-r from-slate-800 via-purple-600 to-indigo-600 text-slate-900 shadow-xl'}`}>{hasLiked ? t('overlay.sent') : t('overlay.like')}</motion.button>
            <motion.button whileTap={{ scale: 0.9 }} onClick={handleChat} className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-slate-900"><span className="material-icons">chat_bubble</span></motion.button>
          </div>
        </div>
        ...
        <div className="fixed top-8 left-0 right-0 px-6 flex justify-between items-center z-[120] pointer-events-none">
          ...
          >
            <span className="material-icons text-slate-900">expand_more</span>
          </button>
          ...
          <div className="relative pointer-events-auto">
            ...
            >
              <span className="material-icons text-slate-900">more_horiz</span>
            </button>
            ...
            <AnimatePresence>
              {showOptions && (
                <motion.div 
                  ...
                  className="absolute top-12 right-0 w-48 bg-white/95 backdrop-blur-3xl border border-white/10 rounded-3xl p-2 shadow-2xl overflow-hidden"
                >
                  <button 
                    onClick={async () => {
